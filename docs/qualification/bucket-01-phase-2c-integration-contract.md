...
The governing design is `B01-PROBE-STABLE-POLAR-V2`.

## Required Phase 2B artifacts

The intake requires four exact-head, Design-V2-bound artifacts:

1. `lafea-bucket-01-probe-stable-candidate-mesh-package/v1`
2. `lafea-bucket-01-probe-stable-candidate-topology-report/v1`
3. `lafea-bucket-01-probe-stable-candidate-validation-evidence/v1`
4. `lafea-bucket-01-probe-stable-topology-validation-evidence/v1`

All four artifacts must carry canonical semantic hashes, the same exact-head SHA, the frozen Design V2 hash, and authority fields that remain false for production, stress acceptance, qualification, and Bucket-01 status.

The validation evidence must be generated by executed checkers. Self-declared package or topology summaries are not sufficient. Candidate intake requires evidence that:

- candidate mesh packages were rebuilt;
- radial and circumferential coordinate hashes were recomputed;
- the Design V2 midside geometry policy was rebuilt and verified;
- feature-set and quality hashes were recomputed;
- location records were rebuilt;
- topology assertions were recomputed from those records.

## Design V2 geometry boundary

The accepted candidate must prove:

- analytic circular midsides occur on the physical hole and outer boundaries;
- internal circumferential midsides are exact straight-chord midpoints;
- radial and diagonal midsides are exact straight-chord midpoints;
- all corner-scaled, integration-point, and dense-sampled Jacobians are positive.

A package using analytic circular midsides on internal radial rings is a Design V1 package and must be rejected even when its identifier or design hash is altered.

## Intake gate

The Phase 2C intake contract verifies:

- exact custody agreement between candidate package, topology report, validation evidence, Design V2, and head SHA;
...
- executed rebuild and recomputation evidence;
- exact compliance with the Design V2 midside geometry policy;
