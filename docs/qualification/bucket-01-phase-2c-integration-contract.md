...
The intake requires four exact-head, design-bound artifacts:
...
All four artifacts must carry canonical semantic hashes, the same exact-head SHA, the frozen Phase 2 design hash, and authority fields that remain false for production, stress acceptance, qualification, and Bucket-01 status.
...
- exact custody agreement between candidate package, topology report, validation evidence, design, and head SHA;
