...
- `check:linear-piping-analysis-consumer`, `check:lfea-interfaces` and `check:lfea-code-application` are registered inside `check:lfea-core`.
...
- Raw project-file ingestion is not yet bound to the complete application result.
- Read-only piping presentation, stale-state containment and byte-deterministic exports are not implemented.
