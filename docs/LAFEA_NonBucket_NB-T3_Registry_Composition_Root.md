...
`lafea-stage-registry/v2` retains the existing stage taxonomy, authority descriptions, engine states, collection paths, preview policies and limitations. Each entry additionally binds one immutable `lafea-stage-composition-binding/v1` record.
...
|---|---|---|---|
| LAFEA.1 | `LAFEA.COMPOSITION.ATTACHMENT_FOUNDATION/V1` | `ANALYTICAL_FOUNDATION_V1` | none registered |
| LAFEA.2 | `LAFEA.COMPOSITION.PIPE_SECTION_SCREENING/V1` | `ANALYTICAL_SCREENING_V1` | none registered |
...
LAFEA.1 through LAFEA.5 retain their existing qualified calculation routes. LAFEA.6 binds only its evidence-neutral placeholder normalizer. It has no canonicalizer, calculator, acceptance rule, result presenter or unit resolver and continues to fail closed with `UNSUPPORTED_STAGE_ENGINE_NOT_IMPLEMENTED`.
...
The bounded NB-T3 check verifies registry/component uniqueness, exact lifecycle-profile binding, registered technical components, retained execution of LAFEA.1 through LAFEA.5, fail-closed LAFEA.6 behavior, benchmark-manifest binding, public contract exposure and absence of duplicate stage dispatch maps.
