...
The latter three files are bounded B0-discovered corrections:

- removal of one shadowed duplicate npm script declaration while retaining the existing effective governed command;
...
## 6. B0-discovered corrections

### 6.1 Duplicate package-script key

The retained aggregate audits detected two declarations of `check:lafea-workbench`. The earlier declaration was shadowed by the later governed command:

```text
node scripts/lafea-p0-ui-containment-check.mjs && node scripts/lafea-workbench-check.mjs
```

B0 removes only the shadowed declaration. The effective npm command and gate ordering remain unchanged.

### 6.2 Sequential-authoring fixture lineage
