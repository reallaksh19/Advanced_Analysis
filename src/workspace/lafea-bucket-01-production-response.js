...
const LEVEL_KEYS = Object.freeze([
  'ordinal', 'elementCount', 'meshSize', 'meshHash', 'recoveryHash',
  'resultHash', 'solverMethod', 'freeDofCount', 'appliedForce',
  'reactionForce', 'appliedMomentZ', 'reactionMomentZ',
  'totalStrainEnergy', 'halfExternalWork', 'energyQualificationAccepted',
]);
...
export function evaluateLafeaBucket01ProductionResponse(inputValue) {
  ...
  if (inputValue.schema !== LAFEA_BUCKET_01_PRODUCTION_RESPONSE_INPUT_SCHEMA) {
    throw responseError('LAFEA_B01_PRODUCTION_RESPONSE_INPUT_SCHEMA_INVALID');
  }
  ...
  const energyConvergence = evaluateLafeaBucket01Convergence({
  ...
  if (Math.max(...reactionMomentErrors) > tolerances.momentEquilibriumRelative) {
  ...
  const base = {
    schema: LAFEA_BUCKET_01_PRODUCTION_RESPONSE_EVIDENCE_SCHEMA,
    producerRevision: LAFEA_BUCKET_01_PRODUCTION_RESPONSE_REVISION,
    exactHeadSha,
    specHash,
    locationDefinitionHash,
    expectedAppliedForce,
...
export function validateLafeaBucket01ProductionResponseEvidence(value) {
  try {
    if (!value
      || value.schema !== LAFEA_BUCKET_01_PRODUCTION_RESPONSE_EVIDENCE_SCHEMA
      || value.producerRevision !== LAFEA_BUCKET_01_PRODUCTION_RESPONSE_REVISION) {
      throw responseError('LAFEA_B01_PRODUCTION_RESPONSE_EVIDENCE_INVALID');
    }
    const rebuilt = evaluateLafeaBucket01ProductionResponse({
    ...
    if (JSON.stringify(rebuilt) !== JSON.stringify(value)) {
      throw responseError('LAFEA_B01_PRODUCTION_RESPONSE_REBUILD_MISMATCH');
    }
    if (!isDeepFrozen(value)) {
      throw responseError('LAFEA_B01_PRODUCTION_RESPONSE_NOT_FROZEN');
    }
    return deepFreeze({ ok: true, errors: [] });
  } catch (error) {
    return deepFreeze({
      ok: false,
      errors: [error?.code ?? 'LAFEA_B01_PRODUCTION_RESPONSE_INVALID'],
    });
  }
}
  ...
  const levels = [...value].sort((left, right) => left.ordinal - right.ordinal)
    .map((row, index) => {
      ...
      return deepFreeze({
        ordinal,
        elementCount,
        meshSize: positive(row.meshSize, 'meshSize'),
...
function normalizeTolerances(value) {
  ...
  exactKeys(value.asymptoticRatioBounds, ASYMPTOTIC_KEYS, 'asymptoticRatioBounds');
  if (value.asymptoticRatioBounds.maximum
    < value.asymptoticRatioBounds.minimum) {
    throw responseError('LAFEA_B01_PRODUCTION_RESPONSE_ASYMPTOTIC_BOUNDS_INVALID');
  }
  return deepFreeze({
    loadResultantRelative: positive(value.loadResultantRelative, 'loadResultantRelative'),
    forceEquilibriumRelative: positive(value.forceEquilibriumRelative, 'forceEquilibriumRelative'),
    loadMomentRelative: positive(value.loadMomentRelative, 'loadMomentRelative'),
    momentEquilibriumRelative: positive(value.momentEquilibriumRelative, 'momentEquilibriumRelative'),
    ...
    minimumObservedOrder: value.minimumObservedOrder === null
      ? null : nonNegative(value.minimumObservedOrder, 'minimumObservedOrder'),
    asymptoticRatioBounds: deepFreeze({
      minimum: positive(value.asymptoticRatioBounds.minimum, 'asymptotic minimum'),
      maximum: positive(value.asymptoticRatioBounds.maximum, 'asymptotic maximum'),
    }),
  });
}
...
function exactKeys(value, keys, label) {
  if (!value || typeof value !== 'object' || Array.isArray(value)
    ...
    throw responseError('LAFEA_B01_PRODUCTION_RESPONSE_RECORD_INVALID', label);
  }
...
function positive(value, label) {
  ...
  if (!(normalized > 0)) {
    throw responseError('LAFEA_B01_PRODUCTION_RESPONSE_POSITIVE_REQUIRED', label);
  }
  return normalized;
}
...
function nonNegative(value, label) {
  const normalized = finite(value, label);
  if (normalized < 0) {
    throw responseError('LAFEA_B01_PRODUCTION_RESPONSE_NONNEGATIVE_REQUIRED', label);
  }
  return normalized;
}
