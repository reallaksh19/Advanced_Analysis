...
} from '../engineering-enrichment/production-pipe-section-overlay.js';
import {
...
export const AUTHORIZED_EMPIRICAL_LOAD_EXECUTION_V6_REQUEST_SCHEMA =
  'authorized-empirical-load-execution-request/v6';
export const AUTHORIZED_EMPIRICAL_LOAD_EXECUTION_V6_SCHEMA =
  'authorized-empirical-load-execution/v6';
...
export function authorizedEmpiricalLoadExecutionV6SemanticProjection(value) {
  return Object.fromEntries(OUTPUT_KEYS
    .filter((key) => key !== 'semanticHash')
    .map((key) => [key, value[key]]));
}

export function computeAuthorizedEmpiricalLoadExecutionV6SemanticHash(value) {
  return semanticHash(authorizedEmpiricalLoadExecutionV6SemanticProjection(value));
}

/**
 * Package 5D production cutover. Existing sealed 5A/5B/5C authorities remain
 * independent; PIPE_SECTIONS is added as a fourth independently current seal.
 *
 * The pipe-section tuple is source-backed and atomic, but this slice authorizes
 * dimensional changes only. materialCode and insulationCode must remain equal
 * to the previously authorized section identities so Package 5D cannot silently
 * select a different material density or insulation-density authority.
 */
export function calculateAuthorizedEmpiricalLoadExecutionV6(value) {
  exact(value, REQUEST_KEYS, 'authorizedEmpiricalLoadExecutionV6Request');
  if (value.schema !== AUTHORIZED_EMPIRICAL_LOAD_EXECUTION_V6_REQUEST_SCHEMA) {
    fail('Unsupported authorized empirical V6 execution request.', 'EMPIRICAL_EXECUTION_V6_SCHEMA_INVALID');
  }
  ...
  const sectionObserved = assertEngineeringEnrichmentObservedAuthority(value.pipeSectionObservedAuthority);

  ready(component, 'component-weight', 'EMPIRICAL_EXECUTION_V6_COMPONENT_ENRICHMENT_BLOCKED');
  ready(operatingFluid, 'operating-fluid-density', 'EMPIRICAL_EXECUTION_V6_FLUID_ENRICHMENT_BLOCKED');
  ready(material, 'material-density', 'EMPIRICAL_EXECUTION_V6_MATERIAL_ENRICHMENT_BLOCKED');
  ready(pipeSection, 'pipe-section', 'EMPIRICAL_EXECUTION_V6_PIPE_SECTION_ENRICHMENT_BLOCKED');
  observedBinding(
    component,
    componentObserved,
    'component-weight',
    'EMPIRICAL_EXECUTION_V6_COMPONENT_OBSERVED_AUTHORITY_MISMATCH',
  );
  observedBinding(
    operatingFluid,
    fluidObserved,
    'operating-fluid-density',
    'EMPIRICAL_EXECUTION_V6_FLUID_OBSERVED_AUTHORITY_MISMATCH',
  );
  observedBinding(
    material,
    materialObserved,
    'material-density',
    'EMPIRICAL_EXECUTION_V6_MATERIAL_OBSERVED_AUTHORITY_MISMATCH',
  );
  observedBinding(
    pipeSection,
    sectionObserved,
    'pipe-section',
    'EMPIRICAL_EXECUTION_V6_PIPE_SECTION_OBSERVED_AUTHORITY_MISMATCH',
  );
  ...
  sourceBinding(pipeSection, sectionObserved, value.dataset, 'pipe-section');
  ...
  if (new Set(contextHashes).size !== 1) {
    fail(
      'Package 5A-5D approvals were reviewed against different context identities.',
      'EMPIRICAL_EXECUTION_V6_CONTEXT_AUTHORITY_MISMATCH',
    );
  }
  ...
  for (const observed of observedAuthorities) {
    if (observed.contextIdentities?.projectDataHash !== activeProjectDataHash) {
      fail(
        ...
        'EMPIRICAL_EXECUTION_V6_PROJECT_DATA_STALE',
        {
  ...
  requireCompleteLineCoverage(
    ...
    'Package 5D requires complete Package 5B operating-fluid-density coverage.',
    'EMPIRICAL_EXECUTION_V6_OPERATING_FLUID_COVERAGE_INCOMPLETE',
  );
  requireCompleteLineCoverage(
    authorizedLineKeys,
    Object.keys(material.lineMaterialDensitiesKgPerM3).sort(ascii),
    'Package 5D requires complete Package 5C material-density coverage.',
    'EMPIRICAL_EXECUTION_V6_MATERIAL_DENSITY_COVERAGE_INCOMPLETE',
  );
  requireCompleteLineCoverage(
    authorizedLineKeys,
    Object.keys(pipeSection.pipeSectionProperties).sort(ascii),
    'Package 5D requires complete sealed pipe-section coverage for every authorized line binding.',
    'EMPIRICAL_EXECUTION_V6_PIPE_SECTION_COVERAGE_INCOMPLETE',
  );
  ...
  const profile = applyMaterialDensityEnrichment(sectionProfile, material, authorizedInput);
  ...
  if (errors.length > 0) {
    fail(
      'The Package 5D sealed-enrichment Project Data profile is not calculation-ready.',
      'EMPIRICAL_EXECUTION_V6_PROFILE_BLOCKED',
      { errors },
    );
  }
  ...
  if (distribution.method !== requestedMethod) {
    fail(
      ...
      'EMPIRICAL_EXECUTION_V6_METHOD_MISMATCH',
  ...
  const effectiveMaterialDensities = projectDataPayload(
    profile.loadCalculation.materialDensitiesKgPerM3,
  );
  const effectivePipeSections = projectDataPayload(profile.loadCalculation.pipeSectionProperties);
  const effectiveInsulationDensities = projectDataPayload(
    profile.loadCalculation.insulationDensitiesKgPerM3,
  );
  const draft = {
    schema: AUTHORIZED_EMPIRICAL_LOAD_EXECUTION_V6_SCHEMA,
    ...
    pipeSectionObservedAuthorityHash: sectionObserved.observedAuthorityHash,
  ...
  return requireAuthorizedEmpiricalLoadExecutionV6({
    ...draft,
    semanticHash: computeAuthorizedEmpiricalLoadExecutionV6SemanticHash(draft),
  });
}

export function requireAuthorizedEmpiricalLoadExecutionV6(value) {
  exact(value, OUTPUT_KEYS, 'authorizedEmpiricalLoadExecutionV6');
  if (value.schema !== AUTHORIZED_EMPIRICAL_LOAD_EXECUTION_V6_SCHEMA) {
    fail('Unsupported authorized empirical V6 execution.', 'EMPIRICAL_EXECUTION_V6_SCHEMA_INVALID');
  }
  const result = {
    ...value,
    executionId: identity(value.executionId, 'executionId'),
  ...
  if (result.requestedMethod !== result.executedMethod
    ...
    fail('Authorized, executed and distribution methods do not agree.', 'EMPIRICAL_EXECUTION_V6_METHOD_MISMATCH');
  }
  if (result.distribution.status !== result.status) {
    fail('Execution status differs from distribution.', 'EMPIRICAL_EXECUTION_V6_STATUS_MISMATCH');
  }
  if (result.distributionSemanticHash !== semanticHash(result.distribution)) {
    fail('Distribution semantic hash is stale.', 'EMPIRICAL_EXECUTION_V6_HASH_MISMATCH');
  }
  if (semanticHash(result.summary) !== semanticHash(summarize(result.distribution))) {
    fail('Execution summary is stale.', 'EMPIRICAL_EXECUTION_V6_SUMMARY_MISMATCH');
  }
  if (result.semanticHash !== computeAuthorizedEmpiricalLoadExecutionV6SemanticHash(result)) {
    fail('Execution semantic hash is stale.', 'EMPIRICAL_EXECUTION_V6_HASH_MISMATCH');
  }
...
function applyPipeSectionEnrichment(profile, enrichment, authorizedInput) {
  ...
  for (const binding of authorizedInput.lineBindings) {
    ...
    if (!baseline || !sealed) {
      fail(
        'Package 5D requires both authorized baseline and sealed pipe section for every line.',
        'EMPIRICAL_EXECUTION_V6_PIPE_SECTION_MISSING',
        { lineKey },
      );
    }
    if (identity(sealed.materialCode, `pipeSectionProperties.${lineKey}.materialCode`)
      ...
      fail(
        'Package 5D may not change materialCode; material selection requires a separately governed cutover.',
        'EMPIRICAL_EXECUTION_V6_MATERIAL_CODE_CHANGE_OUTSIDE_SCOPE',
        { lineKey, baseline: baseline.materialCode, sealed: sealed.materialCode },
      );
    }
    if (identity(sealed.insulationCode, `pipeSectionProperties.${lineKey}.insulationCode`)
        !== identity(baseline.insulationCode, `authorizedPipeSection.${lineKey}.insulationCode`)) {
      fail(
        'Package 5D may not change insulationCode; insulation selection requires a separately governed cutover.',
        'EMPIRICAL_EXECUTION_V6_INSULATION_CODE_CHANGE_OUTSIDE_SCOPE',
        { lineKey, baseline: baseline.insulationCode, sealed: sealed.insulationCode },
      );
    }
...
function applyMaterialDensityEnrichment(profile, enrichment, authorizedInput) {
  ...
  for (const binding of authorizedInput.lineBindings) {
    ...
    if (!section || typeof section !== 'object' || Array.isArray(section)) {
      fail('Authorized line is missing an active pipe section.', 'EMPIRICAL_EXECUTION_V6_PIPE_SECTION_MISSING', { lineKey });
    }
    ...
    if (!positive(densityKgPerM3)) {
      fail('Authorized line is missing a sealed material density.', 'EMPIRICAL_EXECUTION_V6_MATERIAL_DENSITY_MISSING', { lineKey });
    }
    if (Object.prototype.hasOwnProperty.call(materialDensitiesKgPerM3, materialCode)
      ...
      fail(
        ...
        'EMPIRICAL_EXECUTION_V6_MATERIAL_CODE_DENSITY_CONFLICT',
        {
          materialCode,
          existingDensityKgPerM3: materialDensitiesKgPerM3[materialCode],
          densityKgPerM3,
          lineKey,
        },
      );
...
function requireCompleteLineCoverage(expected, actual, message, code) {
  if (JSON.stringify(expected) !== JSON.stringify(actual)) {
    fail(message, code, { expected, actual });
  }
}
...
function sourceBinding(overlay, observed, dataset, label) {
  if (overlay.sourceDatasetHash !== dataset?.sourceSha256
    ...
    fail(
      ...
      'EMPIRICAL_EXECUTION_V6_ENRICHMENT_SOURCE_MISMATCH',
      { label },
    );
  }
}
...
function requireDistribution(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)
    ...
    fail('A support-load distribution is required.', 'EMPIRICAL_EXECUTION_V6_DISTRIBUTION_INVALID');
  }
...
function method(value) {
  if (!AUTHORIZED_EMPIRICAL_EXECUTION_METHODS.includes(value)) {
    fail(
      ...
      'EMPIRICAL_EXECUTION_V6_METHOD_INVALID',
...
function fieldFamilies(value) {
  if (JSON.stringify(value) !== JSON.stringify(FIELD_FAMILIES)) {
    fail(
      'Package 5D execution may activate only 5A component weights, 5B operating fluid, 5C material density and 5D pipe sections.',
      'EMPIRICAL_EXECUTION_V6_FIELD_FAMILY_INVALID',
    );
  }
...
function exact(value, keys, label) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    fail(`${label} must be an object.`, 'EMPIRICAL_EXECUTION_V6_TYPE_INVALID');
  }
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    fail(
      ...
      'EMPIRICAL_EXECUTION_V6_KEYS_INVALID',
      { actual, expected },
    );
  }
}
...
function identity(value, label) {
  if (typeof value !== 'string' || value.trim() !== value || value.length === 0) {
    fail(`${label} must be a non-empty trimmed string.`, 'EMPIRICAL_EXECUTION_V6_IDENTITY_INVALID');
  }
  return value;
}
...
function timestamp(value, label) {
  ...
  if (new Date(result).toISOString() !== result) {
    fail(`${label} must be a canonical ISO-8601 timestamp.`, 'EMPIRICAL_EXECUTION_V6_TIMESTAMP_INVALID');
  }
  return result;
}
...
function hash(value, label) {
  if (typeof value !== 'string' || !/^fnv1a64:[0-9a-f]{16}$/u.test(value)) {
    fail(`${label} must be an FNV-1a semantic hash.`, 'EMPIRICAL_EXECUTION_V6_HASH_INVALID');
  }
  return value;
}
...
function nullableVersion(value) {
  ...
  if ((typeof value !== 'string' && !Number.isInteger(value))
    ...
    fail('datasetVersion must be null, integer or non-empty string.', 'EMPIRICAL_EXECUTION_V6_VERSION_INVALID');
  }
  return value;
}
...
function nonnegativeInteger(value, label) {
  if (!Number.isInteger(value) || value < 0) {
    fail(`${label} must be a non-negative integer.`, 'EMPIRICAL_EXECUTION_V6_NUMBER_INVALID');
  }
  return value;
}
...
function executionStatus(value) {
  if (!['CALCULATED', 'BLOCKED'].includes(value)) {
    fail('Execution status must be CALCULATED or BLOCKED.', 'EMPIRICAL_EXECUTION_V6_STATUS_INVALID');
  }
  return value;
}
