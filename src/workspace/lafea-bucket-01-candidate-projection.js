...
import {
  assertLafeaBucket01CandidateDocumentMatchesMesh,
  buildLafeaBucket01CandidateProjectedLevels,
} from './lafea-bucket-01-candidate-stage-projector.js';
import {
...
export const LAFEA_BUCKET_01_CANDIDATE_PATH_DECLARATION_SCHEMA =
...
export function createLafeaBucket01CandidateProjection(inputValue) {
  ...
  const intake = requireCandidateIntake(
    inputValue.candidateIntakeEvidence,
    exactHeadSha,
    designHash,
  );
  const candidatePackages = requireCandidatePackages(
    inputValue.candidatePackages,
    inputValue.meshProfiles,
    intake,
  );
  ...
  assertReleaseAuthority(
    ...
    canonical.compatibilityReceipt,
  );

  const built = buildLafeaBucket01CandidateProjectedLevels({
  ...
  const sourceInput = deepFreeze(structuredClone({
    ...inputValue,
    candidatePackages,
  }));
  const base = {
...
function requireCandidateIntake(value, exactHeadSha, designHash) {
  if (validateLafeaBucket01ProbeStableCandidateIntakeEvidence(value).ok
      !== true
...
function requireCandidatePackages(packages, meshProfiles, intake) {
  if (!Array.isArray(packages) || packages.length !== 4
    || !Array.isArray(meshProfiles) || meshProfiles.length !== 4) {
    throw candidateError('LAFEA_B01_CANDIDATE_PROJECTION_LEVELS_INVALID');
  }
  return packages.map((packageValue, index) => {
...
function createCandidateMappingPackage({
  ...
  const loadResult = loadPathResult(
    level,
    canonical.physicalProblem,
    canonical.featureProjection,
  );
  const boundaryResult = boundaryPathResult(
    level,
    canonical.featureProjection,
  );
  const loadEdgeEvidence = createContinuumApplicationPathMappingEvidence({
    ...parents,
    kind: 'LOAD_EDGE',
    qualification: loadResult.reasons.length ? 'BLOCK' : 'PASS',
  ...
  const boundaryEdgeEvidence = createContinuumApplicationPathMappingEvidence({
    ...parents,
    kind: 'BOUNDARY_EDGE',
    qualification: boundaryResult.reasons.length ? 'BLOCK' : 'PASS',
...
function candidateDeclaration(level, physicalProblem, featureProjection) {
  return {
    schema: LAFEA_BUCKET_01_CANDIDATE_PATH_DECLARATION_SCHEMA,
    templateId: TEMPLATE_ID,
    stageId: STAGE_ID,
    ordinal: level.ordinal,
...
function loadPathResult(level, physicalProblem, featureProjection) {
...
function boundaryPathResult(level, featureProjection) {
  ...
  if (!restraintSufficient) {
    reasons.push('BOUNDARY_RIGID_BODY_RANK_DEFICIENT');
  }
  return {
    reasons,
    metrics: {
...
function matrixRank(rows, tolerance) {
  ...
  for (let column = 0; column < 3 && rank < matrix.length; column += 1) {
    ...
    for (let columnIndex = column; columnIndex < 3; columnIndex += 1) {
      matrix[rank][columnIndex] /= divisor;
    }
    for (let row = 0; row < matrix.length; row += 1) {
      if (row === rank) continue;
      const factor = matrix[row][column];
      for (let columnIndex = column;
        columnIndex < 3;
        columnIndex += 1) {
        matrix[row][columnIndex] -= factor * matrix[rank][columnIndex];
      }
    }
    rank += 1;
  }
