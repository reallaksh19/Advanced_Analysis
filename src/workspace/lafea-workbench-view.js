...
export class LafeaWorkbenchView {
  ...
  content(state, stage) {
    ...
    const benchmarkCaptions = {
      ...
      'LAFEA.3': `📍 Practical Benchmark (LAFEA.3): ASME B31.3 Reinforced Nozzle/Trunnion Junction with Beveled Collar Pad (${geom.nodes.length} nodes, ${geom.elements.length} elements). 2D axisymmetric plane-stress continuum mesh evaluating stress concentration at weld fillet roots and pad toe bevels.`,
