...
import { assertTopologyEditCertifiedJournal } from './topology-edit-certified-journal.js';
import {
...
function assertEntryBasis(entry, journal, current) {
  const basis = entry.request.basis;
  if (basis.sourceHash !== journal.basis.sourceHash
    || basis.baseCanonicalHash !== journal.basis.baseCanonicalHash) {
    throw new Error(`TopologyEditReplayService: command ${entry.commandId} basis differs from journal authority.`);
  }
  if (basis.priorDraftHash !== current.canonicalTopologyHash) {
...
function compareEntryCertification(entry, certification) {
  ...
  const candidate = certification.candidate;
  if (candidate.candidateDraftHash !== entry.receipt.result.candidateDraftHash
    || candidate.canonicalTopologyHash !== entry.receipt.result.canonicalTopologyHash) {
    throw new Error(`TopologyEditReplayService: command ${entry.commandId} candidate hash drifted.`);
  }
}

function certifyEntry(entry, current, baseCanonicalTopology) {
...
function entryEvidence(entry, topology) {
  return {
...
function replayHistory(journal, baseCanonicalTopology) {
  const snapshots = [baseCanonicalTopology];
  const evidence = [];
  let current = baseCanonicalTopology;
  for (const entry of journal.history) {
    assertEntryBasis(entry, journal, current);
    const certification = certifyEntry(entry, current, baseCanonicalTopology);
    current = certification.candidate.canonicalTopology;
    snapshots.push(current);
    evidence.push(entryEvidence(entry, current));
  }
  return { snapshots, evidence };
}

function buildReplayResult(journal, snapshots, evidence) {
  const activeCanonicalTopology = snapshots[journal.activeCommandIds.length];
...
export function replayTopologyEditCertifiedJournal({ journal: journalInput, baseCanonicalTopology } = {}) {
  ...
  const { snapshots, evidence } = replayHistory(journal, baseCanonicalTopology);
  return buildReplayResult(journal, snapshots, evidence);
}
