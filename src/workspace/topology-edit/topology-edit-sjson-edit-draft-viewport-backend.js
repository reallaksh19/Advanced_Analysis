...
const EDIT_DRAFT_MAX_FITTING_RADIUS_MM = 28;
const EDIT_DRAFT_FITTING_OPACITY = 0.35;
...
export class TopologyEditSjsonEditDraftNavigationHudViewportBackend
  ...
  renderCompactRouteProjection(group, projection, opacity, markerSize) {
    ...
    try {
      for (const segment of projection.compactSegments || []) {
        const material = cachedMaterial(
          materials,
          segment.colorInt,
          compactRouteOpacity(segment, opacity),
        );
        const object = compactSegmentObject(
          segment,
          material,
    ...
    const nodeElements = compactNodeElements(projection);
    this.buildNodePickProxyGroup(group, nodeElements, governedPickMarkerSize);
    ...
    if (group === this.groups.draftGroup && this.hostElement) {
      ...
      this.hostElement.dataset.topologyEditCompactFittingMaximumRadiusMm = String(
        EDIT_DRAFT_MAX_FITTING_RADIUS_MM,
      );
      this.hostElement.dataset.topologyEditCompactFittingOpacity = String(
        EDIT_DRAFT_FITTING_OPACITY,
      );
      this.hostElement.dataset.topologyEditCompactNodePickProxyCount = String(nodeElements.length);
    }
    return bounds;
  }
...
function compactSegmentObject(segment, material, radialSegments) {
  ...
  const sourceRadiusMm = governedPositive(
    segment.radiusMm,
    'TOPOLOGY_EDIT_COMPACT_ROUTE_RADIUS_INVALID',
  );
  const radiusMm = compactDisplayRadius(segment, sourceRadiusMm);
  ...
  if (segment.curveKind === 'CUBIC_BEZIER') {
    ...
    const sourceEndRadiusMm = positive(segment.endRadiusMm) || sourceRadiusMm;
    const endRadiusMm = compactDisplayRadius(segment, sourceEndRadiusMm);
...
function compactNodeElements(projection) {
  const candidates = projection?.compactElements?.length
    ? projection.compactElements
    : (projection?.elements || []);
  return candidates.filter((element) => {
    const kind = String(element?.type || element?.kind || '').toLowerCase();
    return kind === 'node' || element?.pickTarget?.objectKind === 'node';
  });
}

function compactDisplayRadius(segment, radiusMm) {
  return isPipePathSegment(segment)
    ? radiusMm
    : Math.min(radiusMm, EDIT_DRAFT_MAX_FITTING_RADIUS_MM);
}

function compactRouteOpacity(segment, opacity) {
  return isPipePathSegment(segment)
    ? opacity
    : Math.min(opacity, EDIT_DRAFT_FITTING_OPACITY);
}

function isPipePathSegment(segment) {
  return ['PIPE', 'ELBOW', 'ELBOW_ENTRY', 'ELBOW_EXIT'].includes(segment?.kind);
}
