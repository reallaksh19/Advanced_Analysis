/** Canonical topology normalization and hashing used by the Wave 1 kernel. */
import { deepFreeze, semanticHash } from '../../core/shared-piping-model/index.js';

const SORTED_COLLECTIONS = Object.freeze([
  'nodes',
  'edges',
  'junctions',
  'supports',
...
export function rebuildTopologyEditCrosswalk(topology) {
  return {
    ...
    junctionIdToComponentKey: Object.fromEntries(sortByIdentity(topology.junctions).map((junction) => [junction.id, junction.componentKey ?? null])),
    supportIdToEntityId: Object.fromEntries(sortByIdentity(topology.supports).map((support) => [support.id, support.entityId ?? null])),
  };
}
...
export function topologyEditCanonicalHashMaterial(topology) {
  ...
  for (const key of SORTED_COLLECTIONS) clone[key] = sortByIdentity(clone[key]);
