...
function recordRevision(kind, record) {
  return semanticHash({ kind, record });
}
...
function nodeTarget(topology, nodeId, role) {
  ...
  return {
    ...
    role,
    id: nodeId,
    revision: recordRevision('NODE', record),
...
function edgeTarget(topology, edgeId, role) {
  ...
  return {
    ...
    role,
    id: edgeId,
    revision: recordRevision('EDGE', record),
    record,
  };
}
...
function resolveTargets(topology, request) {
  ...
  switch (commandType) {
    case 'MOVE_NODE':
      return { nodes: [nodeTarget(topology, payload.nodeId, 'MOVE')], edges: [], endpointPortKeys: [] };
    case 'MERGE_NODES': {
      const source = nodeTarget(topology, payload.sourceNodeId, 'SOURCE');
      const target = nodeTarget(topology, payload.targetNodeId, 'TARGET');
      assertMergeIsStructurallyBounded(topology, source.id, target.id);
      return { nodes: [source, target], edges: [], endpointPortKeys: [] };
    }
    case 'BRIDGE_GAP':
    case 'ADD_STRAIGHT_ELEMENT': {
      const from = nodeTarget(topology, payload.fromNodeId, 'FROM');
      const to = nodeTarget(topology, payload.toNodeId, 'TO');
      assertNoExistingConnection(topology, from.id, to.id, commandType);
      return { nodes: [from, to], edges: [], endpointPortKeys: [] };
    }
    case 'SPLIT_EDGE': {
      const edge = edgeTarget(topology, payload.edgeId, 'SPLIT');
      ...
      return { nodes: [from, to], edges: [edge], endpointPortKeys: [] };
    }
    case 'DISCONNECT_ENDPOINT': {
      ...
      return { nodes: [node], edges: [edge], endpointPortKeys: endpointPortKeys(edge.record, node.record, payload.endpoint) };
    }
    case 'DELETE_EDGE':
      return { nodes: [], edges: [edgeTarget(topology, payload.edgeId, 'DELETE')], endpointPortKeys: [] };
    default:
      throw new RangeError(`TopologyEditCommandResolver: unsupported command ${commandType}.`);
  }
}

function targetRevisionMap(targets) {
  return Object.fromEntries([...targets.nodes, ...targets.edges]
...
export function resolveTopologyEditCommand({ request: requestInput, canonicalTopology, authority } = {}) {
  ...
  const targets = resolveTargets(canonicalTopology, request);
  const targetRevisions = targetRevisionMap(targets);
