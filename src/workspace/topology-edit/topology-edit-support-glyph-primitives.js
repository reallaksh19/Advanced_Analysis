...
import {
  createRestraintGlyphEvidence,
  restraintGlyphPickTarget,
  supportGlyphPickTarget,
} from './topology-edit-support-glyph-evidence.js';
...
export function addTopologyEditSupportBody(context) {
  const pickTarget = supportGlyphPickTarget(context.supportEvidence);
  const material = materialFor(0x22d3ee);
  addMesh(context.group, new THREE.OctahedronGeometry(context.markerSize * 0.42, 0), material,
    context.origin, null, pickTarget, 'support-body', context.supportEvidence);
  ...
  ), material.clone(), context.origin, null, pickTarget, 'support-ring', context.supportEvidence);
}

export function addTopologyEditRestraintGlyph(context) {
  const restraintId = text(context.restraint?.restraintId, 'RESTRAINT_ID_MISSING', context.fail);
  const family = text(
    context.restraint?.family, 'RESTRAINT_FAMILY_MISSING', context.fail,
  ).toUpperCase();
  const status = text(
    context.restraint?.status, 'RESTRAINT_STATUS_MISSING', context.fail,
  ).toUpperCase();
  const direction = context.restraint.direction
    ? directionVector(context.restraint.direction, context.fail) : null;
  const modelEvidence = createRestraintGlyphEvidence({
    supportId: context.supportId,
    restraintId,
    restraintFamily: family,
    restraintStatus: status,
    sourcePaths: context.restraint.sourcePaths,
    positiveContactPoint: context.restraint.positiveContactPoint,
    negativeContactPoint: context.restraint.negativeContactPoint,
  });
  const pickTarget = restraintGlyphPickTarget(modelEvidence);
  const builder = BUILDERS[family];
  const diagnostic = !builder || status !== 'RESOLVED';
  const row = {
    group: context.group,
    origin: context.origin,
    size: context.markerSize,
    radialSegments: context.radialSegments,
    material: materialFor(diagnostic ? DIAGNOSTIC_COLOR : restraintColor(family)),
    pickTarget,
    modelEvidence,
  ...
  if (diagnostic) addDiagnostic(row);
  else builder(row);
...
function addDiagnostic(context) {
  addMesh(context.group, new THREE.IcosahedronGeometry(context.size * 0.48, 0),
    nextMaterial(context), context.origin, null, context.pickTarget,
    'diagnostic-restraint', context.modelEvidence);
}
function addContactMarker(context, value, role) {
  ...
  ), nextMaterial(context), center, null, context.pickTarget, role, context.modelEvidence);
}
function addPlate(context, center, normal, width, thickness, role) {
  addMesh(context.group, new THREE.BoxGeometry(width, thickness, width * 0.72),
    ...
    context.pickTarget, role, context.modelEvidence);
}
...
function addMesh(group, geometry, material, position, quaternion, pickTarget, partRole, modelEvidence) {
...
function text(value, code, fail) {
  const result = typeof value === 'string' ? value.trim() : '';
  if (!result) fail('A non-empty text value is required.', code);
  return result;
}
