/** Immutable accepted-command journal transitions for Wave 1. */
...
function rejectedCommandTransition(journal, replay, reason, certification = null) {
  return transition({
...
function validateRequestBasis(request, journal, replay) {
  try {
    assertRequestBasis(request, journal, replay.activeCanonicalTopology);
    return null;
  } catch (error) {
...
function certifyRequest(request, journal, replay, baseCanonicalTopology, checkerPolicy) {
  return assertTopologyEditCertificationResult(certifyTopologyEditCommand({
    request,
    canonicalTopology: replay.activeCanonicalTopology,
...
function appendCertifiedCommand(journal, request, certification) {
  ...
  return appendTopologyEditJournalEntry(journal, entry);
}

function acceptedCommandTransition(journal, nextJournal, certification, baseCanonicalTopology) {
...
export function acceptTopologyEditCommand({
  ...
  const replay = replayTopologyEditCertifiedJournal({ journal, baseCanonicalTopology });
  if (journal.redoCommandIds.length) return rejectedCommandTransition(journal, replay, 'REDO_PENDING');

  const basisError = validateRequestBasis(request, journal, replay);
  if (basisError) return rejectedCommandTransition(journal, replay, basisError);

  const certification = certifyRequest(request, journal, replay, baseCanonicalTopology, checkerPolicy);
  if (certification.disposition !== 'ACCEPTED') {
    const reason = certification.receipt.reasons.map((row) => row.code).join(',');
    return rejectedCommandTransition(journal, replay, reason, certification);
  }

  const nextJournal = appendCertifiedCommand(journal, request, certification);
  return acceptedCommandTransition(journal, nextJournal, certification, baseCanonicalTopology);
}
