/** Immutable request contracts for the seven Wave 1 native commands. */
...
const COMMAND_SET = new Set(TOPOLOGY_EDIT_NATIVE_COMMANDS);
...
function finitePoint(value, label = 'position') {
  ...
  if (!Object.values(point).every(Number.isFinite)) {
    fail(`${label} must contain finite x, y and z coordinates.`);
  }
  return point;
}
...
function normalizeMove(payload) {
  return {
    nodeId: requiredText(payload.nodeId, 'MOVE_NODE.nodeId'),
    position: finitePoint(payload.position),
  };
}
...
function normalizeMerge(payload) {
  ...
  if (sourceNodeId === targetNodeId) {
    fail('MERGE_NODES source and target must be different.', RangeError);
  }
...
function normalizeAddedEdge(payload, commandType) {
  ...
  if (fromNodeId === toNodeId) {
    fail(`${commandType} endpoints must be different.`, RangeError);
  }
  return {
    fromNodeId,
    toNodeId,
    ...
    entityType: payload.entityType === null || payload.entityType === undefined
      ? 'PIPE'
      : requiredText(payload.entityType, `${commandType}.entityType`).toUpperCase(),
  };
}
...
function normalizeDisconnect(payload) {
  ...
  if (!ENDPOINTS.has(endpoint)) {
    fail('DISCONNECT_ENDPOINT.endpoint must be FROM or TO.', RangeError);
  }
  return { edgeId: requiredText(payload.edgeId, 'DISCONNECT_ENDPOINT.edgeId'), endpoint };
}

function normalizeDelete(payload) {
  return { edgeId: requiredText(payload.edgeId, 'DELETE_EDGE.edgeId') };
}

const PAYLOAD_NORMALIZERS = Object.freeze({
  MOVE_NODE: normalizeMove,
  MERGE_NODES: normalizeMerge,
  BRIDGE_GAP: normalizeAddedEdge,
  ADD_STRAIGHT_ELEMENT: normalizeAddedEdge,
  SPLIT_EDGE: normalizeSplit,
  DISCONNECT_ENDPOINT: normalizeDisconnect,
  DELETE_EDGE: normalizeDelete,
});
...
function requestMaterial(input) {
  ...
  if (!COMMAND_SET.has(commandType)) {
    fail(`Unsupported command type ${commandType}.`, RangeError);
  }
  return {
    ...
    payload: normalizePayload(commandType, input.payload),
