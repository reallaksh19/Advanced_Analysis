/**
 * Wave 1 canonical command request contracts.
 *
 * This module is renderer- and workspace-independent. It validates only the
 * immutable command intent and its authority basis; target existence and
 * adjacency are resolved by topology-edit-command-resolver.js.
 */
...
function optionalPositiveNumber(value, label) {
  ...
  if (!Number.isFinite(number) || number <= 0) fail(`${label} must be a positive finite number.`, RangeError);
  return number;
}
...
function normalizeExpectedTargetRevisions(value) {
  ...
  return Object.fromEntries(Object.entries(value)
    .map(([key, revision]) => [requiredText(key, 'expected target id'), requiredText(revision, `expectedTargetRevisions.${key}`)])
    .sort(([left], [right]) => left.localeCompare(right)));
}

function twoNodePayload(payload, commandType) {
...
function normalizePayload(commandType, value) {
  ...
  switch (commandType) {
    ...
    case 'ADD_STRAIGHT_ELEMENT':
      return twoNodePayload(payload, commandType);
    case 'SPLIT_EDGE': {
    ...
    case 'DISCONNECT_ENDPOINT': {
      ...
      if (!ENDPOINTS.has(endpoint)) fail('DISCONNECT_ENDPOINT.endpoint must be FROM or TO.', RangeError);
