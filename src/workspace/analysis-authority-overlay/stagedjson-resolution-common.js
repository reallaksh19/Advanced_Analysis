...
export const STAGEDJSON_RESOLUTION_STATUS = deepFreeze({
  DECLARED: 'DECLARED',
  INHERITED: 'INHERITED',
  MISSING: 'MISSING',
});
export const STAGEDJSON_PROCESS_INHERITANCE_POLICY = 'PROHIBIT_ENTITY_ORDER_CARRY_FORWARD';
export const STAGEDJSON_TEMPERATURE_ROLES = deepFreeze(['REFERENCE', 'OPERATING', 'DESIGN']);
...
export function normalizeStagedJsonResolvedField(input, options) {
  const { path, allowedUnits, allowInherited = true, expectedSourceEntityId = null, normalizeValue = clone } = options;
  ...
  if (expectedSourceEntityId !== null && sourceEntityId !== expectedSourceEntityId) fail('STAGEDJSON_RESOLUTION_SOURCE_ENTITY_MISMATCH', `${path} must be declared on ${expectedSourceEntityId}.`);
  ...
  return { status, value: normalizeValue(input.value, `${path}.value`), unit, sourceEntityId, sourceField, fromEntityId, diagnosticCodes, evidence };
}

export function stagedJsonFieldSemanticProjection(field) {
  return { status: field.status, value: field.value, unit: field.unit, sourceEntityId: field.sourceEntityId, sourceField: field.sourceField, fromEntityId: field.fromEntityId, diagnosticCodes: field.diagnosticCodes };
}
export function stagedJsonFieldEvidenceProjection(field) { return { status: field.status, evidence: field.evidence }; }
...
export function normalizeDatasetRef(value, dataset, path = 'datasetRef') {
  ...
  const result = { datasetId: text(value.datasetId, `${path}.datasetId`), sourceId: text(value.sourceId, `${path}.sourceId`), sourceSha256: sha256(value.sourceSha256, `${path}.sourceSha256`), sourceSnapshotSemanticHash: hash(value.sourceSnapshotSemanticHash, `${path}.sourceSnapshotSemanticHash`) };
  if (result.datasetId !== dataset.datasetId || result.sourceId !== dataset.sourceName || result.sourceSha256 !== dataset.sourceSha256 || result.sourceSnapshotSemanticHash !== dataset.sourceSnapshot?.sourceSemanticHash) fail('STAGEDJSON_RESOLUTION_DATASET_STALE', `${path} does not match the active dataset.`);
  return result;
}
...
export function normalizeScope(value, dataset, { entityRequired = false, path = 'scope' } = {}) {
  ...
  if (!dataset.entities.some((entity) => entity.branchId === branchId)) fail('STAGEDJSON_RESOLUTION_BRANCH_ORPHANED', `${path}.branchId is absent from the dataset.`);
...
export function normalizeDiagnostics(value, path) { if (!Array.isArray(value)) fail('STAGEDJSON_RESOLUTION_DIAGNOSTICS_INVALID', `${path} must be an array.`); return value.map((row,index)=>{if(!isPlainRecord(row))fail('STAGEDJSON_RESOLUTION_DIAGNOSTICS_INVALID',`${path}[${index}] must be a record.`);return clone(row);}); }
export function normalizeJsonValue(value, path) { try { return clone(value); } catch { fail('STAGEDJSON_RESOLUTION_VALUE_INVALID', `${path} must be JSON-compatible.`); } }
