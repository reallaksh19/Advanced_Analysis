...
} from './workbench-import-panel.js';

export const LAFEA_T7C_IMPORT_ACCESSORY_PANEL_STATUS =
...
export const LAFEA_T7C_IMPORT_ACCESSORY_PANEL_DESCRIPTOR =
  createLafeaT7cImportAccessoryPanelDescriptor();

export function createLafeaT7cImportAccessoryPanelDescriptor(options = {}) {
  ...
  return Object.freeze({
    ...
    mount(context) {
      const { controller, hostElement } = requireMountContext(context);
      ...
      if (!panel || typeof panel.destroy !== 'function') {
        throw new TypeError('T7C import panel mount must return a destroyable controller.');
      }
      let destroyed = false;
      return Object.freeze({
        destroy() {
...
function requireMountContext(value) {
  requireExactPlainRecord(value, MOUNT_CONTEXT_KEYS, 'T7C accessory panel mount context');
  const { controller, hostElement } = value;
  if (!hostElement || typeof hostElement !== 'object') {
    throw new TypeError('T7C accessory panel hostElement is required.');
  }
  if (hostElement.dataset?.role === 'lafea-benchmark-host') {
    throw new TypeError('The LAFEA benchmark host cannot be reused as a T7C accessory host.');
  }
  requireExactPlainRecord(
    controller,
    CONTROLLER_FACADE_KEYS,
    'T7C accessory panel controller facade',
  );
  if (!Object.isFrozen(controller)) {
    throw new TypeError('T7C accessory panel controller facade must be frozen.');
  }
  for (const method of CONTROLLER_FACADE_KEYS) {
    if (typeof controller[method] !== 'function') {
      throw new TypeError(`T7C accessory panel controller.${method} must be a function.`);
    }
  }
  return value;
}
