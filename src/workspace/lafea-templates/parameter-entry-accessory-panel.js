...
import { mountLafeaT7aParameterPanel } from './parameter-entry-live-panel.js';

export const LAFEA_T7A_PARAMETER_ACCESSORY_PANEL_STATUS =
...
export const LAFEA_T7A_PARAMETER_ACCESSORY_PANEL_DESCRIPTOR =
  createLafeaT7aParameterAccessoryPanelDescriptor();

export function createLafeaT7aParameterAccessoryPanelDescriptor(options = {}) {
  ...
  return Object.freeze({
    ...
    mount(context) {
      ...
      const panel = mountLafeaT7aParameterPanel(hostElement, panelOptions);
      if (!panel || typeof panel.destroy !== 'function') {
        throw new TypeError('T7A parameter panel mount must return a destroyable controller.');
      }
      let destroyed = false;
      return Object.freeze({
        destroy() {
...
function requireMountContext(value) {
  requireExactPlainRecord(value, MOUNT_CONTEXT_KEYS, 'T7A accessory panel mount context');
  const { controller, hostElement } = value;
  if (!hostElement || typeof hostElement !== 'object') {
    throw new TypeError('T7A accessory panel hostElement is required.');
  }
  if (hostElement.dataset?.role === 'lafea-benchmark-host') {
    throw new TypeError('The LAFEA benchmark host cannot be reused as a T7A accessory host.');
  }
  requireExactPlainRecord(
    controller,
    CONTROLLER_FACADE_KEYS,
    'T7A accessory panel controller facade',
  );
  if (!Object.isFrozen(controller)) {
    throw new TypeError('T7A accessory panel controller facade must be frozen.');
  }
  for (const method of CONTROLLER_FACADE_KEYS) {
    if (typeof controller[method] !== 'function') {
      throw new TypeError(`T7A accessory panel controller.${method} must be a function.`);
    }
  }
  return value;
}
