/**
 * Private support utilities for immutable LAFEA edit commands.
 *
 * These functions preserve deterministic document digests, JSON safety,
 * diagnostics and exact-key validation. They do not edit a stage document or
 * create source, lifecycle, execution, result or release authority.
 */
...
export function assertJsonSafe(
  ...
  if (!value || typeof value !== 'object') {
    throw pathError(
      'LAFEA_NON_JSON_VALUE',
      path,
      `${path} must contain JSON-safe data.`,
    );
  }
  if (seen.has(value)) {
  ...
  if (Array.isArray(value)) {
    ...
    if (Object.getPrototypeOf(value) !== Object.prototype) {
      throw pathError(
        'LAFEA_NON_PLAIN_OBJECT',
        path,
        `${path} must use plain JSON objects.`,
      );
    }
...
function stringifyValue(value) {
  ...
  return `{${Object.keys(value)
    .sort()
    .map((key) => `${JSON.stringify(key)}:${stringifyValue(value[key])}`)
    .join(',')}}`;
}
