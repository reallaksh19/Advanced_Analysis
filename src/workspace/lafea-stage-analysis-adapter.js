 ...
 *
 * Adapters describe existing stage authority; they create no engineering evidence
 * and may not widen registry or lifecycle contracts.
 */
...
const MESH = Object.freeze({
  ...
  'LAFEA.3': Object.freeze({
    families: Object.freeze(['T3', 'T6', 'Q8']),
    nodePath: 'nodes',
    elementPath: 'elements',
  }),
  'LAFEA.4': Object.freeze({
    families: Object.freeze(['CST_DKT_TRI3_THIN_SHELL_V1']),
    nodePath: 'nodes',
    elementPath: 'elements',
  }),
...
export function requireLafeaStageAnalysisAdapter(stageId) {
  ...
  if (meshApplicable !== (mesh.families.length > 0)) {
    throw adapterError('LAFEA_STAGE_ANALYSIS_ADAPTER_MESH_PROFILE_MISMATCH');
  }
  ...
  return freeze({
    ...
    results: {
      adapterId: registry.presenterRole ? `PRESENTER:${registry.presenterRole}` : null,
    },
    release: {
      qualified: false,
...
export function lafeaStageAnalysisAdapter(stageId) {
  return requireLafeaStageAnalysisAdapter(stageId);
}
