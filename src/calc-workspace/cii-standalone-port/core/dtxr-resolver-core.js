...
export function canonicalRoot(branchName) {
  if (!branchName) return '';
  let norm = normalizeOwnerBranchName(branchName);
  norm = norm.replace(/\/B\d+$/i, ''); // Strip trailing /B1, /B2, etc.
  norm = norm.replace(/\/+$/, ''); // Strip any leftover trailing slashes
  return norm;
}
...
export function selectEffectiveDtxrEvidence(dtxrPosValue, dtxrPsValue) {
  const dtxrPos = cleanDtxr(dtxrPosValue);
  const dtxrPs = cleanDtxr(dtxrPsValue);
  if (dtxrPos) {
    return { dtxrPos, dtxrPs, effectiveDtxr: dtxrPos, effectiveSource: 'DTXR_POS', fallbackUsed: false };
  }
  if (dtxrPs) {
    return { dtxrPos, dtxrPs, effectiveDtxr: dtxrPs, effectiveSource: 'DTXR_PS_FALLBACK', fallbackUsed: true };
  }
  return { dtxrPos: '', dtxrPs: '', effectiveDtxr: '', effectiveSource: 'NONE', fallbackUsed: false };
}

function emptyDtxrResult(reason = 'not-found') {
  return { canonicalText: '', value: '', dtxrPos: '', dtxrPs: '', effectiveDtxr: '', effectiveSource: 'NONE', fallbackUsed: false, teeDesc: '', cmpSupGap: '', supportTags: [], source: reason, matchedBy: '', matchedKey: '', stagedType: '', componentRefNo: '', confidence: 0, suppressed: false, suppressionReason: '', candidates: [] };
}

function resultFromStagedItem(indexed, { source, matchedBy, matchedKey, purpose, confidence = 1 } = {}) {
  const evidence = selectEffectiveDtxrEvidence(
    indexed?.dtxr || stagedComponentDtxr(indexed?.raw || indexed) || '',
    indexed?.dtxrPs || stagedComponentDtxrPs(indexed?.raw || indexed) || '',
  );
  ...
  const canonicalText = evidence.effectiveDtxr;
  return { ...emptyDtxrResult(source || 'staged'), ...evidence, canonicalText, value: canonicalText, teeDesc: purpose === 'tee-description' ? evidence.dtxrPos : '', cmpSupGap, supportTags: [], source: source || 'staged', matchedBy: matchedBy || '', matchedKey: String(matchedKey || ''), stagedType, componentRefNo: toText(indexed?.attrs?.ComponentRefNo || indexed?.attrs?.COMPONENTREFNO || '').trim(), confidence, _sourcePath: indexed?._path || '' };
}

function resultFromExistingXml(xmlNode, { source, matchedBy, confidence = 0.75 } = {}) {
  const nodeNumber = getXmlNodeProperty(xmlNode, 'NodeNumber');
  const evidence = selectEffectiveDtxrEvidence(
    getXmlNodeProperty(xmlNode, 'DTXR_POS') || getXmlNodeProperty(xmlNode, 'DtxrPos'),
    getXmlNodeProperty(xmlNode, 'DTXR_PS') || getXmlNodeProperty(xmlNode, 'DtxrPs'),
  );
  if (!evidence.effectiveDtxr) return null;
  const sourceName = evidence.effectiveSource === 'DTXR_POS' ? 'xml-dtxr-pos' : 'xml-dtxr-ps-fallback';
  return { ...emptyDtxrResult(source || 'xml-existing'), ...evidence, canonicalText: evidence.effectiveDtxr, value: evidence.effectiveDtxr, source: source || sourceName, matchedBy: matchedBy || sourceName, matchedKey: `NodeNumber:${nodeNumber}`, confidence };
}
...
function suppressIfUnsafe(result, { xmlNode, purpose, rules }) {
  ...
  if (badStagedType || unsafeStagedForWeight || unsafeText) return { ...result, canonicalText: '', value: '', effectiveDtxr: '', effectiveSource: 'NONE', fallbackUsed: false, suppressed: true, suppressionReason: badStagedType ? `unsafe staged type ${stagedType} for ${purpose}` : (unsafeStagedForWeight ? `unsafe staged type ${stagedType} for ${purpose}` : `text suppressed for ${purpose}`) };
  return result;
}
...
export function resolveDtxrForXmlNode({ xmlNode, context, purpose = 'component-description', config = {}, trustExistingXmlDtxr = true } = {}) {
  ...
  if (purpose === 'support-restraint') {
    const xmlNodeNumber = getXmlNodeProperty(xmlNode, 'NodeNumber');
    const xmlNodeName = getXmlNodeProperty(xmlNode, 'NodeName');
    const xmlBranch = xmlAncestorBranchName(xmlNode);
    const xmlPosition = normalizePoint(getXmlNodeProperty(xmlNode, 'Position'));
    const tolerance = toFiniteNumber(config.coordinateToleranceMm, 6.0);
    const xmlRoot = canonicalRoot(xmlBranch);

    let group = [];
    let minXmlDist = Infinity;
    let matchedPosition = null;

    if (stagedIndex && xmlRoot && xmlPosition) {
      for (const item of stagedIndex.items) {
        const itemRoot = canonicalRoot(item.owner);
        if (itemRoot !== xmlRoot) continue;
        ...
        const dist = euclideanDistance(xmlPosition, itemCoord);
        if (dist <= tolerance) {
          group.push({ item, dist, coord: itemCoord });
          if (dist < minXmlDist) {
            minXmlDist = dist;
            matchedPosition = itemCoord;
          }
        }
      }
    }

    const formatCoord = (c) => c ? `${c.x.toFixed(2)} ${c.y.toFixed(2)} ${c.z.toFixed(2)}` : '';
    let record = {
      xmlNodeNumber, xmlNodeName, xmlBranch, xmlPosition: formatCoord(xmlPosition),
      sourceBranch: '', branchRelationship: '', coordinateToleranceMm: tolerance,
      matchedPosition: '', positionDistanceMm: null, maxInternalDistanceMm: null,
      dtxrPosNodeNumbers: '', dtxrPsNodeNumbers: '', dtxrPosValue: '', dtxrPsName: '', dtxrPsValue: '',
      effectiveSource: 'NONE', derivedSupportTypes: '', matchType: 'UNRESOLVED', status: 'UNRESOLVED', sourcePaths: '',
      
      // Legacy compatibility
      canonicalText: '', dtxrPos: '', dtxrPs: '', effectiveDtxr: '', fallbackUsed: false,
      supportTags: xmlNodeSupportTags(xmlNode), matchedBy: '', matchedKey: ''
    };
    ...
    if (group.length > 0) {
      ...
      for (let i = 0; i < group.length; i++) {
        for (let j = i + 1; j < group.length; j++) {
          const d = euclideanDistance(group[i].coord, group[j].coord);
          if (d > maxInternalDist) maxInternalDist = d;
        }
      }

      const dtxrPosVals = new Set();
      const posItems = [];
      const psItems = [];
      let derivedDtxrPs = '';
      let derivedDtxrPsName = '';

      for (const g of group) {
        const pos = cleanDtxr(g.item.dtxr || stagedComponentDtxr(g.item));
        if (pos) {
          dtxrPosVals.add(pos);
          posItems.push(g.item);
        }
      }
      
      const dtxrPosValue = [...dtxrPosVals].join('+');

      if (xmlNodeName) {
        for (const g of group) {
          const itemTags = supportTagsFromAttrs(g.item.attrs, g.item.component?.name || '');
          const itemName = cleanDtxr(g.item.attrs.NAME || g.item.component?.name || '');
          const normXmlName = xmlNodeName.toUpperCase();
          if (itemName.includes(normXmlName) || itemTags.includes(normalizeSupportTag(xmlNodeName))) {
            derivedDtxrPsName = xmlNodeName;
            derivedDtxrPs = cleanDtxr(g.item.dtxrPs || stagedComponentDtxrPs(g.item)) || cleanDtxr(g.item.dtxr || stagedComponentDtxr(g.item));
            psItems.push(g.item);
            break; 
          }
        }
      ...
      const hasPos = !!dtxrPosValue;
      const hasPs = !!derivedDtxrPs;
      const status = hasPos && hasPs ? 'RESOLVED_POS_PS' : (hasPos ? 'RESOLVED_POS' : (hasPs ? 'RESOLVED_PS_FALLBACK' : 'UNRESOLVED'));

      record.sourceBranch = group[0].item.owner || '';
      record.branchRelationship = 'same-root';
      record.matchedPosition = formatCoord(matchedPosition);
      record.positionDistanceMm = minXmlDist;
      record.maxInternalDistanceMm = maxInternalDist;
      record.dtxrPosNodeNumbers = posItems.map(i => i.attrs?.NodeNumber || i._jsonSeq || '').join(',');
      record.dtxrPsNodeNumbers = psItems.map(i => i.attrs?.NodeNumber || i._jsonSeq || '').join(',');
      record.dtxrPosValue = dtxrPosValue;
      record.dtxrPsName = derivedDtxrPsName;
      record.dtxrPsValue = derivedDtxrPs;
      record.effectiveSource = hasPos ? 'DTXR_POS' : (hasPs ? 'DTXR_PS' : 'NONE');
      record.matchType = 'COORDINATE_GROUP';
      record.status = status;
      record.sourcePaths = group.map(g => g.item._path).join('|');
      
      record.canonicalText = hasPos ? dtxrPosValue : derivedDtxrPs;
      record.dtxrPos = dtxrPosValue;
      record.dtxrPs = derivedDtxrPs;
      record.effectiveDtxr = hasPos ? dtxrPosValue : derivedDtxrPs;
      record.fallbackUsed = !hasPos && hasPs;
      record.matchedBy = 'coordinate-group';
      record.matchedKey = `${group.length} items within ${tolerance}mm`;
    }

    const checked = suppressIfUnsafe(record, { xmlNode, purpose, rules });
    return checked.suppressed ? { ...record, status: 'UNRESOLVED', effectiveDtxr: '', canonicalText: '' } : checked;
  }
  ...
  if (purpose === 'support-restraint' && trustExistingXmlDtxr && rules.allowExistingXmlDtxr !== false) {
...
export function applyDtxrAnnotations(document, node, resolved, purpose = 'component-description') {
  ...
  if (purpose === 'support-restraint') {
    ...
    if (resolved.dtxrPos) { xmlSetText(document, node, 'DTXR_POS', resolved.dtxrPos); wroteEvidence = true; }
    if (resolved.dtxrPs) { xmlSetText(document, node, 'DTXR_PS', resolved.dtxrPs); wroteEvidence = true; }
    if (!wroteEvidence && resolved.canonicalText) {
      const field = resolved.effectiveSource === 'DTXR_PS_FALLBACK' ? 'DTXR_PS' : 'DTXR_POS';
      xmlSetText(document, node, field, resolved.canonicalText);
      wroteEvidence = true;
    }
    if (wroteEvidence) { xmlSetText(document, node, 'DTXR_SOURCE', resolved.source); count += 1; }
...
export function resolveXmlCiiNodeDtxr(xmlNode, stagedIndex, config = {}) {
  ...
  return { value: result.canonicalText || result.value || '', dtxrPos: result.dtxrPos || '', dtxrPs: result.dtxrPs || '', effectiveDtxr: result.effectiveDtxr || '', effectiveSource: result.effectiveSource || 'NONE', fallbackUsed: result.fallbackUsed === true, source: result.source, confidence: result.confidence, matchedKey: result.matchedKey, matchedBy: result.matchedBy, stagedType: result.stagedType, suppressed: result.suppressed, suppressionReason: result.suppressionReason, sourcePath: result._sourcePath || '' };
}
...
export function buildStagedDtxrPositionIndex(stagedJsonText, config) {
  ...
  for (const entry of walkStagedComponents(parsed)) {
    ...
    const name = toText(attrs.NAME || attrs.name || entry.component?.name || '').trim();
    ...
    if (!point) continue;
    const indexed = { ...entry, dtxr, dtxrPos, dtxrPs, cmpSupGap, name, point, rawPoint, supportTags, _path: entry._path || '' };
...
export function xmlCiiDtxrPosForNode(node, dtxrPositionIndex, config) {
  ...
  for (const entry of dtxrPositionIndex.entries) {
    ...
    if (entry.dtxr && !seenDtxr.has(entry.dtxr)) {
      seenDtxr.add(entry.dtxr);
      rawValues.push(entry.dtxr);
      const annotParts = [];
      if (entry.name) annotParts.push(`NAME=${entry.name}`);
      if (entry.cmpSupGap) annotParts.push(`CMPSUPGAP=${entry.cmpSupGap}`);
      const annotation = annotParts.length ? `(${annotParts.join(',')})` : '';
      labeledValues.push(annotation ? `${entry.dtxr}${annotation}` : entry.dtxr);
    }
    if (entry.cmpSupGap && !gapValues.includes(entry.cmpSupGap)) gapValues.push(entry.cmpSupGap);
  }
  if (rawValues[0]) annotateComponentDtxr(node, 'POS', rawValues[0]);
  if (gapValues[0]) annotateCmpSupGap(node, 'POS', gapValues[0]);
  return { text: labeledValues.join('|'), values: rawValues, cmpSupGap: gapValues[0] || '', matchedPath };
}
