...
export const LAFEA_LUG_PINHOLE_PROBE_STABLE_T6_V3_PACKAGE_SCHEMA =
  'lafea-lug-pinhole-probe-stable-t6-package/v3';
export const LAFEA_LUG_PINHOLE_PROBE_STABLE_T6_V3_TOPOLOGY_SCHEMA =
  'lafea-lug-pinhole-probe-stable-t6-topology-observation/v3';
export const LAFEA_LUG_PINHOLE_PROBE_STABLE_T6_V3_GENERATOR_REVISION =
  'B01-PROBE-STABLE-T6.3';
export const LAFEA_BUCKET_01_PROBE_STABLE_DESIGN_V3_ID =
  'B01-PROBE-STABLE-POLAR-V3';
...
export function generateLafeaLugPinholeProbeStableT6MeshV3(specValue) {
  const sourcePackage = generateV1Mesh(specValue);
  if (sourcePackage.spec.designId !== LAFEA_BUCKET_01_PROBE_STABLE_DESIGN_V3_ID) {
    throw meshError('LAFEA_B01_CANDIDATE_V3_DESIGN_ID_REQUIRED');
  }
  if (!sourcePackage.spec.radialAxis.protectedBreakpoints.includes(60)) {
    throw meshError('LAFEA_B01_CANDIDATE_V3_MAPPING_BREAKPOINT_REQUIRED');
  }
  ...
  const mappingWindow = createMappingWindowEvidence(sourcePackage.spec, featureSets);
  const mappingWindowHash = canonicalLafeaSha256(mappingWindow);
  const base = {
    schema: LAFEA_LUG_PINHOLE_PROBE_STABLE_T6_V3_PACKAGE_SCHEMA,
    generatorRevision: LAFEA_LUG_PINHOLE_PROBE_STABLE_T6_V3_GENERATOR_REVISION,
...
export function validateLafeaLugPinholeProbeStableT6MeshV3Package(value) {
  try {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
      throw meshError('LAFEA_B01_CANDIDATE_V3_PACKAGE_NOT_RECORD');
    }
    if (value.schema !== LAFEA_LUG_PINHOLE_PROBE_STABLE_T6_V3_PACKAGE_SCHEMA
      || value.generatorRevision
        !== LAFEA_LUG_PINHOLE_PROBE_STABLE_T6_V3_GENERATOR_REVISION
      || value.spec?.designId !== LAFEA_BUCKET_01_PROBE_STABLE_DESIGN_V3_ID) {
      throw meshError('LAFEA_B01_CANDIDATE_V3_PACKAGE_SCHEMA_INVALID');
    }
    ...
    if (JSON.stringify(value.midsideGeometryPolicy)
      ...
      throw meshError('LAFEA_B01_CANDIDATE_V3_MIDSIDE_POLICY_INVALID');
    }
    if (canonicalLafeaSha256(value.mesh) !== value.meshHash
      ...
      throw meshError('LAFEA_B01_CANDIDATE_V3_HASH_MISMATCH');
    }
    verifyMidsidePolicy(value.mesh, value.spec.radialAxis.coordinates.length - 1);
    const independentMappingWindow = createMappingWindowEvidence(
      value.spec,
      value.featureSets,
    );
    if (JSON.stringify(independentMappingWindow)
      !== JSON.stringify(value.mappingWindow)) {
      throw meshError('LAFEA_B01_CANDIDATE_V3_MAPPING_WINDOW_MISMATCH');
    }
    ...
    if (JSON.stringify(independentQuality) !== JSON.stringify(value.quality)
      ...
      throw meshError('LAFEA_B01_CANDIDATE_V3_QUALITY_MISMATCH');
    }
    ...
    if (value.status !== expectedStatus
      ...
      throw meshError('LAFEA_B01_CANDIDATE_V3_STATUS_MISMATCH');
    }
    if (canonicalLafeaSha256(packageSemanticIdentity(value))
      !== value.semanticHash) {
      throw meshError('LAFEA_B01_CANDIDATE_V3_SEMANTIC_HASH_MISMATCH');
    }
    const rebuilt = generateLafeaLugPinholeProbeStableT6MeshV3(value.spec);
    if (rebuilt.semanticHash !== value.semanticHash
      || rebuilt.meshHash !== value.meshHash
      || rebuilt.qualityHash !== value.qualityHash
      || rebuilt.mappingWindowHash !== value.mappingWindowHash) {
      throw meshError('LAFEA_B01_CANDIDATE_V3_REBUILD_MISMATCH');
    }
    if (!isDeepFrozen(value)) {
      throw meshError('LAFEA_B01_CANDIDATE_V3_PACKAGE_NOT_FROZEN');
    }
    return deepFreeze({ ok: true, errors: [] });
  } catch (error) {
    return deepFreeze({
      ok: false,
      errors: [error?.code ?? 'LAFEA_B01_CANDIDATE_V3_PACKAGE_INVALID'],
    });
  }
}

export function observeLafeaLugPinholeProbeStableT6TopologyV3(
  packageValue,
  locationValue,
) {
  if (packageValue?.schema
      !== LAFEA_LUG_PINHOLE_PROBE_STABLE_T6_V3_PACKAGE_SCHEMA) {
    throw meshError('LAFEA_B01_CANDIDATE_V3_TOPOLOGY_PACKAGE_INVALID');
  }
  ...
  const base = {
    ...sourceWithoutHash,
    schema: LAFEA_LUG_PINHOLE_PROBE_STABLE_T6_V3_TOPOLOGY_SCHEMA,
    generatorRevision: LAFEA_LUG_PINHOLE_PROBE_STABLE_T6_V3_GENERATOR_REVISION,
    designId: LAFEA_BUCKET_01_PROBE_STABLE_DESIGN_V3_ID,
...
function createMappingWindowEvidence(spec, featureSets) {
  const coordinates = spec.radialAxis.coordinates;
  const start = 20;
  const end = 60;
  const startIndex = coordinates.indexOf(start);
  const endIndex = coordinates.indexOf(end);
  if (startIndex !== 0 || endIndex <= startIndex) {
    throw meshError('LAFEA_B01_CANDIDATE_V3_MAPPING_WINDOW_ENDPOINT_MISSING');
  }
  const loadLine = featureSets.radialLines.find(
    (row) => row.role === 'RADIAL_QUARTER_0',
  );
  const restraintLine = featureSets.radialLines.find(
    (row) => row.role === 'RADIAL_QUARTER_2',
  );
  if (!loadLine || !restraintLine) {
    throw meshError('LAFEA_B01_CANDIDATE_V3_MAPPING_FEATURE_MISSING');
  }
  const edgeCount = endIndex - startIndex;
  const nodeStart = startIndex * 2;
  const nodeEnd = nodeStart + edgeCount * 2 + 1;
  const loadNodeIds = loadLine.nodeIds.slice(nodeStart, nodeEnd);
  const restraintNodeIds = restraintLine.nodeIds.slice(nodeStart, nodeEnd);
  if (loadNodeIds.length !== edgeCount * 2 + 1
    || restraintNodeIds.length !== edgeCount * 2 + 1) {
    throw meshError('LAFEA_B01_CANDIDATE_V3_MAPPING_WINDOW_NODE_COUNT_INVALID');
  }
  return deepFreeze({
    ...
    radialStart: start,
    radialEnd: end,
    radialStartIndex: startIndex,
    radialEndIndex: endIndex,
    edgeCount,
...
function transformInternalCircumferentialMidsides(sourceMesh, radialCellCount) {
  ...
  for (const node of mesh.nodes) {
    ...
    if (!first || !second) {
      throw meshError('LAFEA_B01_CANDIDATE_V3_MIDSIDE_ENDPOINT_MISSING');
    }
...
function verifyMidsidePolicy(mesh, radialCellCount) {
  ...
  for (const node of mesh.nodes) {
    ...
    if (ring !== 0 && ring !== radialCellCount && chordDistance > 1e-12) {
      throw meshError('LAFEA_B01_CANDIDATE_V3_INTERNAL_MIDSIDE_NOT_CHORDAL');
    }
    if ((ring === 0 || ring === radialCellCount) && !(chordDistance > 1e-12)) {
      throw meshError('LAFEA_B01_CANDIDATE_V3_BOUNDARY_MIDSIDE_NOT_ANALYTIC');
    }
  }
}
...
function evaluateCandidateMeshQuality(mesh, featureSets, spec) {
  ...
  for (const element of mesh.elements) {
    ...
    if (nodes.some((row) => !row)) {
      throw meshError('LAFEA_B01_CANDIDATE_V3_MESH_NODE_MISSING');
    }
...
function boundaryRadiusError(feature, nodeById, center, expectedRadius) {
  ...
  for (const nodeId of feature.nodeIds) {
    ...
    if (!node) throw meshError('LAFEA_B01_CANDIDATE_V3_FEATURE_NODE_MISSING');
...
function assertCandidateAuthority(value) {
  if (!value
    ...
    throw meshError('LAFEA_B01_CANDIDATE_V3_PRODUCTION_AUTHORITY_FORBIDDEN');
  }
}
