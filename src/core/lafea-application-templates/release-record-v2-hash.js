/** Strict canonical JSON and synchronous SHA-256 for template release identities. */
export const LAFEA_TEMPLATE_RELEASE_HASH_PROFILE = 'LAFEA_CANONICAL_JSON_SHA256_V1';

export function canonicalTemplateReleaseJson(value) {
  return stringifyValue(value, new WeakSet(), '$');
}

export function templateReleaseSha256(value) {
  return `sha256:${sha256Hex(utf8Bytes(canonicalTemplateReleaseJson(value)))}`;
}
  ...
  try {
    if (Array.isArray(value)) {
      ...
      return `[${value.map((entry, index) => {
        ...
        return stringifyValue(descriptor.value, seen, `${path}[${index}]`);
      }).join(',')}]`;
    }
    ...
    return `{${[...keys].sort(codeUnitCompare).map((key) => {
