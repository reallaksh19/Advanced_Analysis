...
export const FLANGE_HUB_TRANSITION_CANDIDATE_SCHEMA =
  'flange-hub-transition-candidate/v2';
export const FLANGE_HUB_TRANSITION_CANDIDATE_FAMILY_ID =
  'BKT-B-FLANGE-Q8-B03-B04-CONFORMING-TRANSITION-V2';
export const FLANGE_HUB_TRANSITION_CANDIDATE_LEVELS =
  FLANGE_HUB_MESH_LEVELS;
...
export const FLANGE_HUB_TRANSITION_CANDIDATE_POLICY = deepFreeze({
  candidateId: FLANGE_HUB_TRANSITION_CANDIDATE_FAMILY_ID,
  authority: 'NON_AUTHORIZING_PRODUCTION_CANDIDATE',
...
export function createFlangeHubTransitionCandidateMesh(
  levelId,
  ...
  const production = createFlangeHubMesh(levelId, geometry);
  ...
  const remappedNodes = production.nodes.map((node) => {
  ...
  const elements = production.elements.map((element) => deepFreeze({
    ...element,
    nodeIds: element.nodeIds.map((nodeId) => (
      merge.aliasByNodeId.get(nodeId) ?? nodeId
    )),
  }));
  const boundaryEdges = production.boundaryEdges.map((edge) => deepFreeze({
    ...edge,
  ...
  const productionQuality = new Map(
    production.quality.elementQuality.map((row) => [row.elementId, row]),
  );
  const qualityRows = elements.map((element) => {
    ...
    if (!quality.accepted) {
      throw new RangeError(
        `FH_TRANSITION_CANDIDATE_QUALITY_REJECTED:`
  ...
  const candidateMetadata = deepFreeze({
    schema: FLANGE_HUB_TRANSITION_CANDIDATE_SCHEMA,
    policy: FLANGE_HUB_TRANSITION_CANDIDATE_POLICY,
    productionMeshFamilyId: production.meshFamilyId,
    productionMeshHash: production.meshHash,
    productionCanonicalModelHash: production.canonicalModelHash,
    productionNodeCount: production.nodeCount,
  ...
  const payload = {
    ...
    moduleId: production.moduleId,
    meshFamilyId: FLANGE_HUB_TRANSITION_CANDIDATE_FAMILY_ID,
    levelId,
    refinement: production.refinement,
    geometryHash: production.geometryHash,
...
export function createFlangeHubTransitionCandidateFamily(
  geometry = createCanonicalFlangeHubGeometry(),
) {
  const levels = FLANGE_HUB_TRANSITION_CANDIDATE_LEVELS.map(
    ({ levelId }) => createFlangeHubTransitionCandidateMesh(
      levelId,
      geometry,
    ),
  );
  const payload = {
    schema: 'flange-hub-transition-candidate-family/v2',
    meshFamilyId: FLANGE_HUB_TRANSITION_CANDIDATE_FAMILY_ID,
    geometryHash: geometry.semanticHash,
    levels,
