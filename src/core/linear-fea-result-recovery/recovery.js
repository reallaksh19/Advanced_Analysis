 ...
 * fields and, when the recovery profile explicitly requests them, component
 * code-point resultants from one sealed `fea-linear-execution/v1` (B-3.3).
 * A force/displacement-only workflow may explicitly defer code-point output;
 * element actions and force fields remain fully recovered from the exact same
 * effective component stiffness used in the solve.
 */
export function compileResultRecovery({
  ...
  const recoverCodePoints = acceptedProfile.recoverComponentCodePoints;
  const nodalLoadByNode = recoverCodePoints ? buildNodalLoadIndex(acceptedLoadCase) : null;
  const componentResultants = [...componentByComponentId.keys()].sort(compareAscii).map((componentId) => {
    ...
    const codePoints = recoverCodePoints
      ? component.codeStations.map((station) => recoverComponentCodePoint({
        station,
        componentElementIds,
        modelElementsById,
        actionByElementId,
