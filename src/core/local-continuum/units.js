...
export function canonicalizeUnits(value){const row=exactRecord(value,['length','force','stress','modulus'],'units');const declared={},conversionFactors={};for(const key of Object.keys(FACTORS)){const unit=row[key];if(typeof unit!=='string'||!Object.hasOwn(FACTORS[key],unit))throw modelError('UNSUPPORTED_UNIT',`units.${key}`,`Unsupported ${key} unit.`);declared[key]=unit;conversionFactors[key]=FACTORS[key][unit];}
  // Body-force intensity (force/volume) is a derived dimension (stress/length)
  // composed from the already-declared stress and length units, rather than
  // a new top-level declared unit key.
  conversionFactors.bodyForceIntensity=conversionFactors.stress/conversionFactors.length;
