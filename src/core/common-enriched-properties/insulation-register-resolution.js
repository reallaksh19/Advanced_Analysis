...
export const COMMON_ENRICHED_INSULATION_RESOLUTION_SCHEMA = 'common-enriched-insulation-resolution/v1';
export const COMMON_ENRICHED_INSULATION_KEY_CONFIG_SCHEMA = 'common-enriched-insulation-key-config/v1';
export const COMMON_ENRICHED_INSULATION_FIELD_BINDING_SCHEMA = 'common-enriched-insulation-field-binding/v1';
export const INSULATION_VALUE_KINDS = Object.freeze(['NUMBER', 'STRING', 'BOOLEAN']);
...
const KEY_CONFIG_KEYS = Object.freeze(['schema', 'targetInsulationCodeField', 'sourceInsulationCodeField']);
...
export function insulationResolutionSemanticProjection(value) {
  return Object.fromEntries(RESOLUTION_KEYS
    .filter((key) => key !== 'semanticHash')
    .map((key) => [key, value[key]]));
}

export function computeInsulationResolutionSemanticHash(value) {
  return semanticHash(insulationResolutionSemanticProjection(value));
}

export function createCommonEnrichedInsulationResolution(input) {
  requireExactKeys(
    input,
    ['schema', 'resolutionId', 'lineListResolution', 'snapshot', 'keyConfig', 'bindings'],
    'insulationResolutionDraft',
  );
  if (input.schema !== COMMON_ENRICHED_INSULATION_RESOLUTION_SCHEMA) schemaError('insulationResolutionDraft');
  ...
  if (snapshot.sourceKind !== 'INSULATION_REGISTER') {
    failCommonEnrichment(
      'insulation resolution requires an INSULATION_REGISTER snapshot.',
      'COMMON_ENRICHED_INSULATION_SOURCE_INVALID',
      { sourceKind: snapshot.sourceKind },
    );
  }
  const keyConfig = requireCommonEnrichedInsulationKeyConfig(input.keyConfig);
  const bindings = normalizeBindings(input.bindings);
  const index = buildExactIndex(snapshot.records, keyConfig.sourceInsulationCodeField);
  const classifications = [];

  const targetRecords = upstream.targetRecords.map((lineRecord) => {
    const key = resolveTargetKey(lineRecord, keyConfig.targetInsulationCodeField);
    if (key.status !== 'EXACT') {
  ...
  const draft = {
    ...
    resolutionId: requireIdentity(input.resolutionId, 'insulationResolution.resolutionId'),
  ...
  return deepFreeze({ ...draft, semanticHash: computeInsulationResolutionSemanticHash(draft) });
}

export function requireCommonEnrichedInsulationResolution(value) {
  requireExactKeys(value, RESOLUTION_KEYS, 'insulationResolution');
  if (value.schema !== COMMON_ENRICHED_INSULATION_RESOLUTION_SCHEMA) schemaError('insulationResolution');
  const result = {
    schema: value.schema,
    resolutionId: requireIdentity(value.resolutionId, 'insulationResolution.resolutionId'),
    ...
    targetRecords: requireUniqueSorted(value.targetRecords, 'targetId', 'insulationResolution.targetRecords')
  ...
  if (JSON.stringify(result.summary) !== JSON.stringify(expectedSummary)) {
    failCommonEnrichment(
      'insulationResolution.summary is stale.',
      'COMMON_ENRICHED_SUMMARY_MISMATCH',
      { expected: expectedSummary, actual: result.summary },
    );
  }
  const expectedHash = computeInsulationResolutionSemanticHash(result);
  if (result.semanticHash !== expectedHash) {
    failCommonEnrichment(
      'insulationResolution.semanticHash is stale.',
...
export function requireCommonEnrichedInsulationKeyConfig(value) {
  requireExactKeys(value, KEY_CONFIG_KEYS, 'insulationKeyConfig');
  if (value.schema !== COMMON_ENRICHED_INSULATION_KEY_CONFIG_SCHEMA) schemaError('insulationKeyConfig');
  return deepFreeze({
    schema: value.schema,
    targetInsulationCodeField: requireIdentity(value.targetInsulationCodeField, 'insulationKeyConfig.targetInsulationCodeField'),
    sourceInsulationCodeField: requireIdentity(value.sourceInsulationCodeField, 'insulationKeyConfig.sourceInsulationCodeField'),
  });
}

export function requireCommonEnrichedInsulationFieldBinding(value) {
  requireExactKeys(value, BINDING_KEYS, 'insulationFieldBinding');
  if (value.schema !== COMMON_ENRICHED_INSULATION_FIELD_BINDING_SCHEMA) schemaError('insulationFieldBinding');
  return deepFreeze({
    schema: value.schema,
    targetField: requireIdentity(value.targetField, 'insulationFieldBinding.targetField'),
    sourceField: requireIdentity(value.sourceField, 'insulationFieldBinding.sourceField'),
    unit: requireNullableUnit(value.unit, 'insulationFieldBinding.unit'),
    valueKind: requireMember(value.valueKind, INSULATION_VALUE_KINDS, 'insulationFieldBinding.valueKind'),
  });
}

function normalizeBindings(value) {
  const bindings = requireUniqueSorted(value, 'targetField', 'insulationResolution.bindings')
    .map(requireCommonEnrichedInsulationFieldBinding);
  if (bindings.length === 0) {
    failCommonEnrichment(
      'insulation resolution requires at least one field binding.',
  ...
  if (new Set(pairs).size !== pairs.length) {
    failCommonEnrichment('Duplicate insulation field binding.', 'COMMON_ENRICHED_DUPLICATE_IDENTITY');
  }
...
function buildExactIndex(records, sourceField) {
  const index = new Map();
  requireArray(records, 'insulationSnapshot.records').forEach((record, recordIndex) => {
    const raw = record.values?.[sourceField];
    if (typeof raw !== 'string' || !raw.trim()) {
      failCommonEnrichment(
        'INSULATION_REGISTER master record has an invalid exact insulation code.',
...
function resolveTargetKey(lineRecord, fieldName) {
  ...
  let failure = null;
  if (!field) failure = ['BLOCKED_MISSING', `INSULATION_REGISTER_KEY_FIELD_MISSING_${fieldName}`];
  else if (field.status !== 'RESOLVED_EXACT') {
    failure = [keyBlockingStatus(field.status), `INSULATION_REGISTER_KEY_NOT_EXACT_${fieldName}_${field.status}`];
  } else if (typeof field.value !== 'string' || !field.value.trim()) {
    failure = ['BLOCKED_CONFLICT', `INSULATION_REGISTER_KEY_TYPE_CONFLICT_${fieldName}`];
  } else if (!field.approved) {
    failure = ['BLOCKED_CONFLICT', `INSULATION_REGISTER_KEY_NOT_APPROVED_${fieldName}`];
  }
  if (failure) {
    return deepFreeze({
      status: failure[0],
      diagnostics: Object.freeze(['INSULATION_REGISTER_KEY_BLOCKED', failure[1]].sort(compareAscii)),
    });
  }
...
function exactField(binding, record, snapshot) {
  ...
  const evidence = {
    sourceKind: 'INSULATION_REGISTER', sourceKey: snapshot.sourceKey, sourceHash: snapshot.sourceHash,
    locator: `${record.locator}:${binding.sourceField}`,
  };
  if (value === null || value === undefined || value === '') {
    return field(binding, null, 'BLOCKED_MISSING', evidence, 'EXACT_INSULATION_CODE_FIELD_MISSING', false,
      ['INSULATION_REGISTER_FIELD_MISSING']);
  }
  if (!matches(value, binding.valueKind)) {
    return field(binding, null, 'BLOCKED_CONFLICT', evidence, 'EXACT_INSULATION_CODE_FIELD_TYPE_CONFLICT', false,
      ['INSULATION_REGISTER_FIELD_TYPE_CONFLICT']);
  }
  return field(binding, value, 'RESOLVED_EXACT', evidence, 'EXACT_INSULATION_CODE_AND_FIELD', true, []);
}
...
function missingRow(binding, key) {
  return field(binding, null, 'BLOCKED_MISSING', noEvidence(), 'NONE', false,
    ['INSULATION_REGISTER_EXACT_ROW_MISSING', `INSULATION_REGISTER_EXACT_KEY_${key.label}`]);
}
...
function ambiguousRow(binding, records, snapshot) {
  ...
  }, 'EXACT_INSULATION_CODE_MULTIPLE_ROWS', false,
  ['INSULATION_REGISTER_EXACT_ROW_AMBIGUOUS', `INSULATION_REGISTER_EXACT_ROW_COUNT_${records.length}`]);
}
...
function summarizeFromRecords(records) {
  ...
  const exactMethods = new Set([
    'EXACT_INSULATION_CODE_AND_FIELD', 'EXACT_INSULATION_CODE_FIELD_MISSING',
    'EXACT_INSULATION_CODE_FIELD_TYPE_CONFLICT', 'EXACT_INSULATION_CODE_MULTIPLE_ROWS',
  ]);
  return deepFreeze({
    lineTargetCount: records.length,
    exactKeyCount: records.filter((_record, index) =>
      [...methods[index]].some((method) => exactMethods.has(method))
      || diagnostics[index].has('INSULATION_REGISTER_EXACT_ROW_MISSING')).length,
    blockedKeyCount: diagnostics.filter((set) => set.has('INSULATION_REGISTER_KEY_BLOCKED')).length,
    missingRowCount: diagnostics.filter((set) => set.has('INSULATION_REGISTER_EXACT_ROW_MISSING')).length,
    ambiguousRowCount: methods.filter((set) => set.has('EXACT_INSULATION_CODE_MULTIPLE_ROWS')).length,
    exactFieldCount: statuses.filter((status) => status === 'RESOLVED_EXACT').length,
    blockedFieldCount: statuses.filter((status) => status.startsWith('BLOCKED_')).length,
    conflictFieldCount: statuses.filter((status) => status === 'BLOCKED_CONFLICT').length,
  });
...
function normalizeSummary(value) {
  requireExactKeys(value, SUMMARY_KEYS, 'insulationResolution.summary');
  return deepFreeze(Object.fromEntries(SUMMARY_KEYS.map((key) => [
    key, requireNonNegativeInteger(value[key], `insulationResolution.summary.${key}`),
  ])));
}
...
function requireLineRecords(records) {
  for (const record of records) {
    if (record.targetKind !== 'LINE' || record.lineKey === null
      ...
      failCommonEnrichment(
        'insulation resolution contains an inconsistent line target.',
...
function hash(value, fieldName) {
  return requireSemanticHash(value, `insulationResolution.${fieldName}`);
}
function noEvidence() {
  return { sourceKind: 'NONE', sourceKey: null, sourceHash: null, locator: null };
}
function canonicalCode(value) {
  return requireIdentity(String(value).trim().toUpperCase(), 'insulationRegisterKey.insulationCode');
}
