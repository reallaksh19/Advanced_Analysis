...
import { parseGovernedInputXmlSourceBundle } from './inputxml-source-binding.js';
import {
...
/**
 * Preserve the current-main standalone graph diagnostic authority. This API
 * consumes the smaller source bundle produced by
 * geometry/adapters/inputxml-model-health-source.js and never reparses XML.
 */
...
/** Preserve the current-main standalone proximity diagnostic authority. */
...
export function diagnoseInputXmlLinearModelHealthContext(content, options) {
  ...
  const sourceBundle = parseGovernedInputXmlSourceBundle(content, ingestionOptions);
