...
const OUTPUT = path.join(
  ROOT,
  'reports/qualification/topology-edit-c3d-wave1-closing-evidence.json',
);
const QUALIFIED_FILES = Object.freeze([
  ...
  'src/workspace/viewport-presentation/topology-edit-section-model.js',
...
if (actualHead !== expectedHead) {
  throw new Error(`Qualification head mismatch: expected ${expectedHead}, received ${actualHead}.`);
}

const files = {};
for (const repositoryPath of QUALIFIED_FILES) {
  const bytes = await readFile(path.join(ROOT, repositoryPath));
  files[repositoryPath] = Object.freeze({
    sha256: sha256(bytes),
...
const evidenceBase = {
  schema: 'TopologyEditC3DWave1ClosingEvidence.v1',
  repository: 'reallaksh19/Advanced_Analysis',
  targetHead: actualHead,
  baseRef: process.env.GITHUB_BASE_REF || null,
  generatedAt: new Date().toISOString(),
  qualification: {
    status: 'PASSED_WAVE1_COMPLETE',
    command: process.env.TOPOLOGY_EDIT_C3D_QUALIFICATION_COMMAND || null,
    scope: 'VISIBILITY_ISOLATION_SECTIONING_AND_WAVE1_QUALIFICATION',
    authorityBoundary: 'DISPLAY_ONLY_NO_CANONICAL_COMMAND_SCOPE_OR_EXPORT_MUTATION',
    contracts: [
      'EXACT_CANONICAL_ID_VISIBILITY',
      'INSTANCED_PICK_TABLE_PRESERVED',
      'ENGINEERING_COORDINATE_SECTION_BOX',
      'CLIPPED_HIT_REJECTION_BEFORE_PICK_RESOLUTION',
      'PRESENTATION_STATE_ROUND_TRIP',
      'DETERMINISTIC_RESET',
      'IDEMPOTENT_SECTION_CLEANUP',
      'NO_CAMERA_DEPENDENCE',
    ],
  },
  environment: {
