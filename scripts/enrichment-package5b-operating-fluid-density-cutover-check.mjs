...
const FLUID_RESOLUTION_HASH = hash('P5B:EXACT_FLUID_RESOLUTION:L-1');
...
assert.equal(fluidCandidate.schema, ENRICHMENT_OPERATING_FLUID_DENSITY_PROJECTION_SCHEMA);
assert.equal(assertEnrichmentOperatingFluidDensityProjection(fluidCandidate), fluidCandidate);
assert.equal(fluidCandidate.summary.status, 'READY_FOR_STRUCTURAL_IMPACT');
assert.equal(fluidCandidate.fluidResolutionHash, FLUID_RESOLUTION_HASH);

const componentGovernance = governedSeal(
  componentCandidate,
  'Package 5A component-weight authority.',
);
const fluidGovernance = governedSeal(
  fluidCandidate,
  'Package 5B operating-fluid-density authority.',
);
...
assert.equal(fluidOverlay.fluidResolutionHash, FLUID_RESOLUTION_HASH);
...
assert.deepEqual(repeated, enrichedV2, 'Package 5B execution must be deterministic');
...
const staleFluidCurrentness = evaluateEngineeringInputSealCurrentness({
  seal: fluidGovernance.seal,
  observedAuthority: staleFluidObserved,
});
const staleFluidOverlay = buildEnrichmentProductionOperatingFluidDensityOverlay({
  seal: fluidGovernance.seal,
  currentness: staleFluidCurrentness,
  candidateProjection: fluidCandidate,
  dataset: fixtureInput.dataset,
});
assert.equal(staleFluidOverlay.status, 'BLOCKED');
assert.deepEqual(staleFluidOverlay.operatingFluidDensitiesKgPerM3, {});
assert.equal(hasBlocker(staleFluidOverlay, 'ENRICHMENT_PRODUCTION_OPERATING_FLUID_SEAL_NOT_CURRENT'), true);
...
const missingLineCandidate = operatingFluidProjection(fixtureInput.dataset, [
  fluidProjectionRow('L-MISSING', 1000, 'FLUID_DENSITY:L-MISSING'),
]);
...
console.log(JSON.stringify({
  ...
  status: 'PASS',
  projectionSchema: fluidCandidate.schema,
