...
export const BM2_M031_ACTIVE_SET_PROFILE = Object.freeze({
  schema: 'lfea-bm2-unilateral-active-set-profile/v1',
  contactTypeCode: '14',
  contactDof: 'UY',
  permittedGapDirection: '+Y',
...
function teeAuthorities(authorities, entries) {
  ...
  for (const junction of authorities.junctions) {
    ...
    const fittingQuality = junction.sifTypeCode === 3 ? 'VERIFIED_B16_9' : 'IMPERFECT_OR_DAMAGED';
    const factors = calculateB31JWeldingTeeFactors({
    ...
    const planeNormal = unit(cross(runDirection, junction.branchDirectionAway), `junction ${junction.centerNodeId} plane normal`);
    ...
    records.push(Object.freeze({
      ...
      sourceApplicability: fittingQuality === 'VERIFIED_B16_9'
        ? 'B31J_WELDING_TEE_WITHIN_DECLARED_SOURCE_CLASS'
        : 'B31J_DIRECTIONAL_EQUATIONS_APPLIED_CONDITIONALLY_TO_SOURCE_TYPE_5',
    }));
  }
...
function junctionMechanics(tees, material) {
  ...
  for (const tee of tees) {
    ...
    directionalCorrectionByElement.set(tee.branchStub.elementId, Object.freeze({
      role: 'BRANCH',
      inPlane: tee.factors.flexibility.branch.inPlane,
      outOfPlane: tee.factors.flexibility.branch.outOfPlane,
      torsional: tee.factors.flexibility.branch.torsional,
...
function plusYCandidateNodeIds(geometry) {
  return Object.freeze(geometry.nodes
    .filter((node) => (node.meta?.restraints ?? []).some((row) => row.typeCode === BM2_M031_ACTIVE_SET_PROFILE.contactTypeCode))
    .map((node) => node.id)
    .sort(compareIds));
}

function constraintDeclarations(geometry, activePlusY) {
  const rows = new Map();
  ...
  for (const node of geometry.nodes) {
    ...
    for (const restraint of node.meta?.restraints ?? []) {
      if (restraint.typeCode === '14' && activePlusY.has(node.id)) add(node.id, 'UY', 'PLUS-Y-ACTIVE');
...
function compileModel(authorities, entries, mechanics, activePlusY, iteration) {
  ...
  return compileMechanicalModel({
    ...
    sourceSemanticHash: semanticHash({
      ...
      activePlusY: [...activePlusY].sort(compareIds),
    ...
    constraintDeclarations: constraintDeclarations(authorities.geometry, activePlusY),
...
function compileElements(authorities, entries, mechanics, loadCase) {
  ...
  return Object.freeze(entries.map((entry) => {
    ...
    const correction = bendCorrection ?? junctionCorrection;
    return Object.freeze({
      entry,
      frameElement,
      correction,
...
function activeSetLedger(execution, candidateNodeIds, activeSet) {
  const reactionScale = Math.max(
    1,
    ...candidateNodeIds.map((nodeId) => Math.abs(valueAt(execution.reactions, `BM2.N${nodeId}`, 'UY'))),
  );
  return Object.freeze(candidateNodeIds.map((nodeId) => {
    const active = activeSet.has(nodeId);
    const gap = clean(valueAt(execution.displacement, `BM2.N${nodeId}`, 'UY'));
    const reaction = active ? clean(valueAt(execution.reactions, `BM2.N${nodeId}`, 'UY')) : 0;
...
function solveIteration(authorities, entries, mechanics, candidateNodeIds, activeSet, label, thermal, iteration) {
  ...
  const ledger = activeSetLedger(execution, candidateNodeIds, activeSet);
  return Object.freeze({
    compilation,
    loadCase,
    elementRecords,
...
function solvePhysicalCase(authorities, entries, mechanics, candidateNodeIds, label, thermal) {
  let activeSet = new Set(candidateNodeIds);
  const history = [];
  ...
  for (let iteration = 0; iteration < BM2_M031_ACTIVE_SET_PROFILE.maximumIterations; iteration += 1) {
    ...
    history.push(Object.freeze({
      ...
      activeNodeIds: Object.freeze([...activeSet].sort(compareIds)),
      transitions,
    ...
    transitions.deactivate.forEach((nodeId) => next.delete(nodeId));
    transitions.activate.forEach((nodeId) => next.add(nodeId));
    activeSet = next;
  }
...
function endpointAction(action, record, nodeId) {
  if (record.entry.sourceSegment.startNodeId === nodeId) {
    return Object.freeze({ local: action.local.I, global: action.global.I });
  }
  if (record.entry.sourceSegment.endNodeId === nodeId) {
    return Object.freeze({ local: action.local.J, global: action.global.J });
  }
  return null;
}

function transferEndpoint(analysis, transfer) {
  const elementId = `BM2.${transfer.elementId}`;
  const action = actionIndex(analysis).get(elementId);
  if (!action) throw new Error(`BM2 transfer action ${elementId} is unavailable.`);
  return Object.freeze({ local: action.local[transfer.end], global: action.global[transfer.end] });
}

function pairAction(authorities, analysis, group) {
  if (group.elementIds.length === 0) {
    const endpoint = transferEndpoint(analysis, group.transferAction);
    return Object.freeze({
  ...
  const from = candidates.map(({ action, record }) => endpointAction(action, record, physicalFrom)).find(Boolean);
  const to = candidates.map(({ action, record }) => endpointAction(action, record, physicalTo)).find(Boolean);
  if (!from || !to) {
...
function derivedLedger(operating, sustained) {
  const susByNode = new Map(sustained.ledger.map((row) => [row.nodeId, row]));
  return Object.freeze(operating.ledger.map((ope) => {
    const sus = susByNode.get(ope.nodeId);
    return Object.freeze({
      nodeId: ope.nodeId,
...
function buildReport(authorities, entries, mechanics, sustained, operating) {
  ...
  const elements = [...authorities.pairGroups.values()]
    ...
    .map((group) => {
      ...
      return Object.freeze({
        ...
        sustained: pairAction(authorities, sustained, group),
        operating: pairAction(authorities, operating, group),
      });
    });
  ...
  return Object.freeze({
    ...
    counts: Object.freeze({
      ...
      plusYCandidates: plusYCandidateNodeIds(authorities.geometry).length,
    }),
    limitations: Object.freeze([
      Object.freeze({ code: 'BM2_UNKNOWN_RESTRAINT_TYPE_15_OMITTED', cause: 'Corrected restraint type 15 remains unclassified and is omitted rather than guessed.' }),
      Object.freeze({ code: 'BM2_TYPE5_JUNCTION_SOURCE_APPLICABILITY_CONDITIONAL', cause: 'Source SIF type 5 receives the B31J directional equation kernel without claiming verified B16.9 fitting quality.' }),
...
export function solveBm2InputXmlQualified() {
  ...
  const candidates = plusYCandidateNodeIds(authorities.geometry);
