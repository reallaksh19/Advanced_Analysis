...
const guardedFiles = [
  ...
  'src/workspace/analysis-authority-overlay/material-section-resolution.js',
];
...
const materialSection = source[guardedFiles[3]];
const contractAndExtraction = `${overlay}\n${subset}\n${branchExtraction}`;
...
reject(/elasticModulus|massDensity|secondMoment|polarMoment|thermalExpansionCoefficient|outerDiameter|wallThickness/iu, 'engineering properties must not be embedded in contract or extraction files', contractAndExtraction);
...
reject(/from\s*['"][^'"]*(?:linear-fea-solver|linear-fea-model-compiler|linear-fea-material|linear-fea-section)[^'"]*['"]/u, 'analysis implementation imports are prohibited in contract or extraction files', contractAndExtraction);
...
reject(/Math\.PI[\s\S]{0,120}(?:outerDiameter|wallThickness)|(?:secondMoment|polarMoment)\s*=/u, 'derived section mechanics must remain in resolvePipeSection', materialSection);
...
assert.match(materialSection, /resolveLinearFeaMaterialState[^}]*sealMaterialTable[\s\S]*from\s*'\.\.\/\.\.\/core\/linear-fea-material\/index\.js'/u);
assert.match(materialSection, /resolvePipeSection[\s\S]*from\s*'\.\.\/\.\.\/core\/linear-fea-section\/index\.js'/u);
assert.match(materialSection, /resolveLinearFeaMaterialState\s*\(/u, 'material authority must call the B-2.2 resolver');
assert.match(materialSection, /resolvePipeSection\s*\(/u, 'section authority must call the B-2.3 resolver');
assert.match(materialSection, /computePipeSectionRequestSemanticHash\s*\(/u, 'section requests must reuse the canonical request hash');
assert.match(materialSection, /semanticHash\s*\(/u, 'source evidence must reuse the canonical semantic hash');
for (const code of [
...
assert.equal(packageJson.scripts['check:w11.4'], 'node scripts/w11.4-material-section-resolution-check.mjs');
...
const w114 = aggregate.indexOf('check:w11.4');
...
assert.ok(w114 > w113, 'check:w11.4 must be registered after check:w11.3');
