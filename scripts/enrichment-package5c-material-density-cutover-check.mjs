...
import {
  ...
  AUTHORIZED_EMPIRICAL_LOAD_EXECUTION_V5_SCHEMA,
  calculateAuthorizedEmpiricalLoadExecutionV5,
  requireAuthorizedEmpiricalLoadExecutionV5,
...
import {
  ...
  ENRICHMENT_PRODUCTION_MATERIAL_DENSITY_OVERLAY_SCHEMA,
...
assert.equal(materialCandidate.schema, ENRICHMENT_MATERIAL_DENSITY_PROJECTION_SCHEMA);
assert.equal(assertEnrichmentMaterialDensityProjection(materialCandidate), materialCandidate);
assert.equal(materialCandidate.summary.status, 'READY_FOR_STRUCTURAL_IMPACT');
assert.equal(materialCandidate.materialResolutionHash, MATERIAL_RESOLUTION_HASH);
...
assert.equal(materialOverlay.schema, ENRICHMENT_PRODUCTION_MATERIAL_DENSITY_OVERLAY_SCHEMA);
assert.equal(materialOverlay.status, 'READY_FOR_PRODUCTION_CONSUMPTION');
assert.deepEqual(materialOverlay.activatedFieldFamilies, ['MATERIAL_DENSITIES']);
assert.deepEqual(materialOverlay.lineMaterialDensitiesKgPerM3, { 'L-1': 8000 });
assert.equal(materialOverlay.bindings[0].lineKey, 'L-1');
assert.equal(materialOverlay.bindings[0].densityKgPerM3, 8000);
assert.equal(materialOverlay.policy.materialDensitiesActivated, true);
assert.equal(materialOverlay.policy.pipeSectionsActivated, false);
assert.equal(materialOverlay.policy.operatingFluidDensitiesActivated, false);
assert.equal(assertEnrichmentProductionMaterialDensityOverlay(materialOverlay), materialOverlay);
...
const baselineV2 = executeV4(EMPIRICAL_LOAD_METHOD);
const enrichedV2 = executeV5(EMPIRICAL_LOAD_METHOD);
const baselineCog = executeV4(EMPIRICAL_LOAD_COG_METHOD);
const enrichedCog = executeV5(EMPIRICAL_LOAD_COG_METHOD);

assert.equal(enrichedV2.schema, AUTHORIZED_EMPIRICAL_LOAD_EXECUTION_V5_SCHEMA);
assert.equal(enrichedV2.status, 'CALCULATED');
assert.deepEqual(enrichedV2.activatedEnrichmentFieldFamilies, [
  'COMPONENT_WEIGHTS', 'OPERATING_FLUID_DENSITIES', 'MATERIAL_DENSITIES',
]);
assert.equal(enrichedV2.materialDensitySealHash, materialGovernance.seal.sealHash);
assert.equal(enrichedV2.materialDensityOverlayHash, materialOverlay.overlayHash);
...
const annulusAreaM2 = Math.PI * ((0.1 ** 2) - (0.09 ** 2)) / 4;
const expectedMetalMassDeltaKg = annulusAreaM2 * (8000 - 7850);
const expectedMetalForceDeltaN = expectedMetalMassDeltaKg * 9.81;
const expectedReactionDeltaN = expectedMetalForceDeltaN / 2;
...
for (const [baseline, enriched] of [[baselineV2, enrichedV2], [baselineCog, enrichedCog]]) {
  for (const caseId of ['EMPTY', 'OPE', 'HYD']) {
    ...
    assertClose(deltas[0], expectedReactionDeltaN);
    assertClose(deltas[1], expectedReactionDeltaN);
    assertClose(sum(deltas), expectedMetalForceDeltaN);

    const baselinePipe = contribution(baseline, caseId, 'PIPE-1');
    const enrichedPipe = contribution(enriched, caseId, 'PIPE-1');
    assertClose(enrichedPipe.massKg - baselinePipe.massKg, expectedMetalMassDeltaKg);
    ...
    assertClose(enrichedPipe.formula.insulationKg, baselinePipe.formula.insulationKg);
...
assert.equal(enrichedV2.effectiveHydroFluidDensitiesSemanticHash, baselineV2.effectiveHydroFluidDensitiesSemanticHash);
assert.equal(
  enrichedV2.effectivePipeSectionPropertiesSemanticHash,
  semanticHash(authorizedInput.loadCalculationOverlay.pipeSectionProperties),
  'Pipe-section authority must remain unchanged.',
);
assert.equal(enrichedV2.effectiveMaterialDensitiesSemanticHash, semanticHash({ 'MAT-1': 8000 }));
assert.notEqual(
  enrichedV2.effectiveMaterialDensitiesSemanticHash,
  semanticHash(authorizedInput.loadCalculationOverlay.materialDensitiesKgPerM3),
);

const repeated = executeV5(EMPIRICAL_LOAD_METHOD);
assert.deepEqual(repeated, enrichedV2, 'Package 5C execution must be deterministic');
assert.equal(JSON.stringify(fixtureInput.dataset), datasetSnapshot, '5C mutated source dataset');
assert.equal(JSON.stringify(fixtureInput.profile), profileSnapshot, '5C mutated Project Data');
assert.equal(JSON.stringify(authorizedInput), authorizedSnapshot, '5C mutated authorized input');

const staleMaterialObserved = buildEnrichmentObservedAuthority({
  ...materialGovernance.packet.evidenceRefs,
  numericalImpactHash: hash('material-impact-stale'),
  contextIdentities: materialGovernance.packet.contextIdentities,
});
assert.throws(
  () => executeV5(EMPIRICAL_LOAD_METHOD, { materialObservedAuthority: staleMaterialObserved }),
  (error) => error.code === 'EMPIRICAL_EXECUTION_V5_MATERIAL_OBSERVED_AUTHORITY_MISMATCH',
);
const staleMaterialCurrentness = evaluateEngineeringInputSealCurrentness({
  seal: materialGovernance.seal,
  observedAuthority: staleMaterialObserved,
});
const staleMaterialOverlay = buildEnrichmentProductionMaterialDensityOverlay({
  seal: materialGovernance.seal,
  currentness: staleMaterialCurrentness,
  candidateProjection: materialCandidate,
  dataset: fixtureInput.dataset,
});
assert.equal(staleMaterialOverlay.status, 'BLOCKED');
assert.deepEqual(staleMaterialOverlay.lineMaterialDensitiesKgPerM3, {});
assert.equal(hasBlocker(staleMaterialOverlay, 'ENRICHMENT_PRODUCTION_MATERIAL_DENSITY_SEAL_NOT_CURRENT'), true);

const changedProfile = structuredClone(fixtureInput.profile);
changedProfile.revision += 1;
assert.throws(
  () => executeV5(EMPIRICAL_LOAD_METHOD, { profile: changedProfile }),
  (error) => error.code === 'EMPIRICAL_EXECUTION_V5_PROJECT_DATA_STALE',
);

const differentContextGovernance = governedSeal(
  materialCandidate,
  'Package 5C alternate-context authority.',
  { selectorRegistryHash: hash('different-selectors') },
);
const differentContextOverlay = buildEnrichmentProductionMaterialDensityOverlay({
...
assert.throws(
  () => executeV5(EMPIRICAL_LOAD_METHOD, {
    sealedMaterialDensityOverlay: differentContextOverlay,
    materialObservedAuthority: differentContextGovernance.observed,
  }),
  (error) => error.code === 'EMPIRICAL_EXECUTION_V5_CONTEXT_AUTHORITY_MISMATCH',
);

const duplicateMaterialCandidate = materialDensityProjection(fixtureInput.dataset, [
  materialProjectionRow('L-1', 8000, 'MATERIAL_DENSITY:L-1:A'),
  materialProjectionRow('L-1', 8010, 'MATERIAL_DENSITY:L-1:B'),
]);
const duplicateGovernance = governedSeal(duplicateMaterialCandidate, 'Duplicate material candidate test.');
const duplicateOverlay = buildEnrichmentProductionMaterialDensityOverlay({
  seal: duplicateGovernance.seal,
  currentness: duplicateGovernance.currentness,
  candidateProjection: duplicateMaterialCandidate,
  dataset: fixtureInput.dataset,
});
assert.equal(duplicateOverlay.status, 'BLOCKED');
assert.deepEqual(duplicateOverlay.lineMaterialDensitiesKgPerM3, {});
assert.equal(hasBlocker(duplicateOverlay, 'ENRICHMENT_PRODUCTION_MATERIAL_DENSITY_LINE_DUPLICATE'), true);

const missingLineCandidate = materialDensityProjection(fixtureInput.dataset, [
  materialProjectionRow('L-MISSING', 8000, 'MATERIAL_DENSITY:L-MISSING'),
]);
const missingLineGovernance = governedSeal(missingLineCandidate, 'Missing material line test.');
const missingLineOverlay = buildEnrichmentProductionMaterialDensityOverlay({
...
assert.equal(hasBlocker(missingLineOverlay, 'ENRICHMENT_PRODUCTION_MATERIAL_DENSITY_LINE_MISSING'), true);
...
const twoLineFluidCandidate = operatingFluidProjection(fixtureInput.dataset, [
  fluidProjectionRow('L-1', 1000, 'FLUID_DENSITY:L-1:CONFLICT'),
  fluidProjectionRow('L-2', 1000, 'FLUID_DENSITY:L-2:CONFLICT'),
]);
const twoLineMaterialCandidate = materialDensityProjection(fixtureInput.dataset, [
  materialProjectionRow('L-1', 8000, 'MATERIAL_DENSITY:L-1:CONFLICT'),
  materialProjectionRow('L-2', 8010, 'MATERIAL_DENSITY:L-2:CONFLICT'),
]);
const twoLineFluidGovernance = governedSeal(twoLineFluidCandidate, 'Two-line fluid authority.');
const twoLineMaterialGovernance = governedSeal(twoLineMaterialCandidate, 'Two-line material conflict authority.');
...
assert.equal(twoLineFluidOverlay.status, 'READY_FOR_PRODUCTION_CONSUMPTION');
assert.equal(twoLineMaterialOverlay.status, 'READY_FOR_PRODUCTION_CONSUMPTION');
assert.throws(
  () => executeV5(EMPIRICAL_LOAD_METHOD, {
  ...
  (error) => error.code === 'EMPIRICAL_EXECUTION_V5_MATERIAL_CODE_DENSITY_CONFLICT',
);

const tamperedOverlay = structuredClone(materialOverlay);
tamperedOverlay.lineMaterialDensitiesKgPerM3['L-1'] = 7999;
assert.throws(
  () => assertEnrichmentProductionMaterialDensityOverlay(tamperedOverlay),
  (error) => error.code === 'ENRICHMENT_PRODUCTION_MATERIAL_DENSITY_HASH_MISMATCH',
);

const tamperedExecution = structuredClone(enrichedV2);
tamperedExecution.activatedEnrichmentFieldFamilies.push('PIPE_SECTIONS');
assert.throws(
  () => requireAuthorizedEmpiricalLoadExecutionV5(tamperedExecution),
  (error) => error.code === 'EMPIRICAL_EXECUTION_V5_FIELD_FAMILY_INVALID',
);

const overlaySource = await readFile(
  new URL('../src/workspace/engineering-enrichment/production-material-density-overlay.js', import.meta.url),
  'utf8',
);
const executionSource = await readFile(
  new URL('../src/workspace/engineering-loads/authorized-empirical-load-execution-v5.js', import.meta.url),
  'utf8',
);
assert.doesNotMatch(
  overlaySource,
  /calculateSupportLoadDistribution|support-load-distribution|linear-fea|lafea|lfea/iu,
  'Material-density production overlay must not contain calculation mechanics.',
);
assert.doesNotMatch(
  executionSource,
  /function\s+(distributePoint|distributeUniform|componentMass|resolveCaseMass|fluidMass|annulusAreaM2)\b/iu,
  'V5 must reuse existing gravity mechanics rather than duplicating formulas.',
);

console.log(JSON.stringify({
  check: 'enrichment-package5c-material-density-cutover',
  status: 'PASS',
  projectionSchema: materialCandidate.schema,
  overlaySchema: materialOverlay.schema,
  executionSchema: enrichedV2.schema,
  baselineMaterialDensityKgPerM3: authorizedInput.loadCalculationOverlay.materialDensitiesKgPerM3['MAT-1'],
  sealedMaterialDensityKgPerM3: 8000,
  expectedMetalMassDeltaKg,
  expectedMetalForceDeltaN,
  expectedReactionDeltaN: [expectedReactionDeltaN, expectedReactionDeltaN],
  emptyOpeHydAllShiftByMetalSelfWeightOnly: true,
  componentMechanicsPreserved: true,
  operatingFluidAuthorityPreserved: true,
  hydroAuthorityPreserved: true,
  pipeSectionAuthorityPreserved: true,
  independentSealContextsRequiredToMatch: true,
  sharedMaterialCodeConflictFailsClosed: true,
  staleAuthorityFailsClosed: true,
  sourceImmutable: true,
}, null, 2));

function executeV4(method, overrides = {}) {
  return calculateAuthorizedEmpiricalLoadExecutionV4({
    schema: AUTHORIZED_EMPIRICAL_LOAD_EXECUTION_V4_REQUEST_SCHEMA,
    executionId: `BASE-5B:${method}`,
    executedAt: '2026-08-08T07:12:00.000Z',
    method,
    authorizedInput,
...
function executeV5(method, overrides = {}) {
  return calculateAuthorizedEmpiricalLoadExecutionV5({
    ...
    executionId: `ENRICHED-5C:${method}`,
    executedAt: '2026-08-08T07:13:00.000Z',
    method,
    authorizedInput,
    sealedComponentWeightOverlay: componentOverlay,
    componentObservedAuthority: componentGovernance.observed,
...
function governedSeal(candidateValue, basis, contextOverrides = {}) {
  ...
  const approval = buildEngineeringEnrichmentApproval({
    ...
    approvedAt: '2026-08-08T07:09:00.000Z',
    basis,
  });
  const seal = buildEngineeringInputSeal({
    ...
    sealedAt: '2026-08-08T07:10:00.000Z',
  });
...
function fluidProjectionRow(targetId, proposedValue, proposalId) {
  ...
  return Object.freeze({
    ...
    proposalHash: semanticHash({
      fluidResolutionHash: FLUID_RESOLUTION_HASH,
      targetKind: 'LINE', targetId, fieldId: 'fluid.densityKgM3', proposedValue, unit: 'kg/m3', sourceEvidence,
    }),
...
function materialProjectionRow(targetId, proposedValue, proposalId) {
  ...
  return Object.freeze({
    ...
    proposalHash: semanticHash({
      materialResolutionHash: MATERIAL_RESOLUTION_HASH,
      targetKind: 'LINE', targetId, fieldId: 'material.densityKgM3', proposedValue, unit: 'kg/m3', sourceEvidence,
    }),
...
function fixture(cogPointMm) {
  return {
    dataset: {
      ...
      datasetId: 'EMP-PROD-05C-DATASET',
      version: 1,
...
function makeAuthorizedInput({ includeSecondLine = false } = {}) {
  const pipeSectionProperties = {
    'L-1': { outsideDiameterMm: 100, wallThicknessMm: 5, materialCode: 'MAT-1', insulationCode: 'INS-1', insulationThicknessMm: 10 },
  };
  ...
  if (includeSecondLine) {
    ...
    lineBindings.push({
      targetId: 'line:002', sourceRecordId: 'SOURCE-PIPE-2', lineKey: 'L-2',
  ...
  const draft = {
    ...
    intakeId: includeSecondLine ? 'INTAKE-EMP-05C-TWO-LINES' : 'INTAKE-EMP-05C',
    projectId: 'EMP-PROD-05C-PROJECT',
    baselineId: 'BASELINE-EMP-05C', baselineRevision: 1,
    baselineSemanticHash: 'fnv1a64:1111111111111111', readinessEvaluationSemanticHash: 'fnv1a64:2222222222222222',
    readinessSemanticHash: 'fnv1a64:3333333333333333', handoffSemanticHash: 'fnv1a64:4444444444444444',
    projectionPayloadSemanticHash: 'fnv1a64:5555555555555555', adapterVersion: 'empirical-adapter/1.0.0',
    configurationHash: 'fnv1a64:6666666666666666', createdAt: '2026-08-08T07:05:00.000Z',
    lineBindings,
    componentBindings: [{
    ...
    summary: {
      lineCount: lineBindings.length, componentCount: 1, materialCodeCount: 1,
...
function makeProfile() {
  ...
  const sourced = (value, sourceKey, sourceHash) => createEvidenceValue(value, { source: 'EMP_PROD_05C_FIXTURE', sourceKey, sourceHash }, true);
  return {
    ...empty,
    projectId: 'EMP-PROD-05C-PROJECT', revision: 1, updatedAt: '2026-08-08T07:04:00.000Z',
    sourcesAndUnits: {
    ...
    topology: {
      ...empty.topology,
      portMatchToleranceMm: approved(1, 'EMP_PROD_05C_TOPOLOGY'),
      supportSiteGroupingToleranceMm: approved(1, 'EMP_PROD_05C_TOPOLOGY'),
      autoCarrierCoincidenceToleranceMm: approved(1, 'EMP_PROD_05C_TOPOLOGY'),
      routeJoiningRules: approved({ mode: 'EXACT' }, 'EMP_PROD_05C_TOPOLOGY'),
      supportTypeCapabilities: approved({ REST: { vertical: true } }, 'EMP_PROD_05C_TOPOLOGY'),
    },
    loadCalculation: {
      ...empty.loadCalculation,
      gravityMPerS2: approved(9.81, 'EMP_PROD_05C_LOAD_POLICY'),
      loadFactor: approved(1, 'EMP_PROD_05C_LOAD_POLICY'),
      equilibriumTolerances: approved({ forceN: 1e-8, momentNmm: 1e-5 }, 'EMP_PROD_05C_EQUILIBRIUM'),
      activeLoadCases: approved(['EMPTY', 'OPE', 'HYD'], 'EMP_PROD_05C_CASES'),
    },
  };
}
