...
for (const [stageId, families] of Object.entries(expected)) {
  ...
  assert.equal(adapter.preparation.qualified, false);
  assert.equal(adapter.release.qualified, false);
  assert.ok(Object.isFrozen(adapter));
}
assert.equal(
  requireLafeaStageAnalysisAdapter('LAFEA.3').discretization.sourceNodePath,
  'nodes',
);
assert.equal(
  requireLafeaStageAnalysisAdapter('LAFEA.5').discretization.sourceNodePath,
  'shellTemplate.nodes',
);
...
assert.ok(stage.orchestration.sections.SOURCE.reasons.includes(
...
assert.deepEqual(stage.orchestration.sections.RELEASE.reasons, [
...
store.subscribe(() => {
  publications += 1;
  throw new Error('subscriber failure must remain isolated');
});
...
assert.ok(stage.orchestration.sections.PREPARATION.reasons.includes(
  'LAFEA_PREFEA_BRIDGE_NOT_QUALIFIED',
));
assert.equal(stage.orchestration.sections.DISCRETIZATION.state, 'NOT_STARTED');
assert.equal(stage.orchestration.sections.AUTHORIZATION.state, 'BLOCKED');
assert.ok(stage.orchestration.sections.AUTHORIZATION.reasons.includes(
  'LAFEA_PREFEA_BRIDGE_NOT_QUALIFIED',
));
...
assert.deepEqual(
  store.buildOrchestrationProjection(stageId),
  stage.orchestration,
);
...
console.log(JSON.stringify({
  check: 'lafea-architecture-convergence',
  status: 'PASS',
  stageAdapters: Object.keys(expected),
  orchestrationSchema: LAFEA_WORKBENCH_ORCHESTRATION_SCHEMA,
  onePublicStoreBoundary: true,
  sourceLifecycleMeshProjectedTogether: true,
  preparationAuthorityManufactured: false,
  meshGenerationAuthorized: false,
