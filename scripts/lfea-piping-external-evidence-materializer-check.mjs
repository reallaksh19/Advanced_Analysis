...
try {
  ...
  assert.equal(resultOne.status, 'ELIGIBLE_FOR_WP2_BINDING');
  ...
  const authorityHeadMismatch = fixture(path.join(temp, 'authority-head-mismatch'));
  authorityHeadMismatch.projectAuthorityIndex.candidate.sha =
    'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb';
  writeJson(
    path.join(
      authorityHeadMismatch.inputRoot,
      authorityHeadMismatch.request.projectAuthorityIndex,
    ),
    authorityHeadMismatch.projectAuthorityIndex,
  );
  rejected(
    authorityHeadMismatch,
    path.join(temp, 'authority-head-mismatch-output'),
    'LFEA_EXTERNAL_MATERIALIZATION_AUTHORITY_HEAD_MISMATCH',
  );
...
function fixture(root) {
  ...
  const projectAuthorityIndex = {
    schema: 'lfea-piping-phase6i-project-authority-index/v1',
    candidate: { sha: HEAD, ref: 'release/lfea-piping-phase6i-synthetic' },
