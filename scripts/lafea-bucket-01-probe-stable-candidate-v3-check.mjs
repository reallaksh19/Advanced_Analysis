...
import {
  LAFEA_BUCKET_01_PROBE_STABLE_DESIGN_V3_ID,
...
const REPORT_PATH = path.resolve(
  ROOT,
  process.env.LAFEA_BUCKET_01_PROBE_STABLE_V3_REPORT_PATH
    ?? 'reports/qualification-diagnostics/lafea-bucket-01-probe-stable-candidate-v3.json',
);
...
try {
  ...
  state.reasons.push(error?.code ?? error?.message ?? 'UNEXPECTED_V3_FAILURE');
...
function execute() {
  assert.equal(design.schema, 'lafea-bucket-01-probe-stable-polar-design/v3');
  assert.equal(design.designId, LAFEA_BUCKET_01_PROBE_STABLE_DESIGN_V3_ID);
  ...
  const expectedCounts = [480, 1190, 4080, 14256];
  const packages = [];
  for (let index = 0; index < design.levelCount; index += 1) {
    ...
    const packageValue = generateLafeaLugPinholeProbeStableT6MeshV3(spec);
    ...
    assert.equal(packageValue.mappingWindow.radialStart, 20);
    assert.equal(packageValue.mappingWindow.radialEnd, 60);
    assert.equal(packageValue.mappingWindow.exactEndpointNodes, true);
    assert.equal(packageValue.mappingWindow.physicalCoordinateSelection, true);
    assert.equal(packageValue.mappingWindow.indexScaledSelectionUsed, false);
    assert.equal(packageValue.mappingWindow.loadFeatureRole, responseSpec.load.featureRole);
    assert.equal(
      packageValue.mappingWindow.restraintFeatureRole,
    ...
    state.levels.push({
      ...
      mappingWindow: packageValue.mappingWindow,
      status: 'PASS',
    });
  }
  ...
  assert.equal(intakeEvidence.authority.executedRecomputation, true);
  assert.equal(intakeEvidence.authority.independentCheckerExecution, false);
  assert.equal(
    intakeEvidence.authority.independentCheckerRequiredBeforeReplayAdjudication,
    true,
  );
  assert.equal(
  ...
  return {
    schema: 'lafea-bucket-01-probe-stable-candidate-v3-check/v1',
    status: 'PASS',
    exactHeadSha,
...
function verifyLocations(packages) {
  for (const location of allLocations()) {
    const observations = packages.map((packageValue) =>
      observeLafeaLugPinholeProbeStableT6TopologyV3(packageValue, location));
...
function candidateSpec(radialLevel, circumferentialLevel) {
  return {
    ...
    meshIdentity: `B01-PROBE-STABLE-V3-L${radialLevel.ordinal}`,
    designId: design.designId,
