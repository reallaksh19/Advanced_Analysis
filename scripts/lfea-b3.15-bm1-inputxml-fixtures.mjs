...
function compileCase(authorities, loadCaseId, thermal) {
  const primitives = [{ schema: 'fea-linear-load-primitive/v1', primitiveId: `${loadCaseId}-GRAVITY`, kind: 'GRAVITY', direction: { x: 0, y: 1, z: 0 }, basis: 'GLOBAL', includedMassSources: ['PIPE_WALL', 'CONTENTS', 'INSULATION'], sourceEvidence: sourceEvidence({ sourceId: `${SOURCE_ID}-GRAVITY`, sourceRevision: loadCaseId }) }];
  for (const entry of authorities.modelEntries) {
    ...
    const insulationMass = analysis.insulationDensity * Math.PI * (insulatedOuterDiameter ** 2 - entry.section.dimensions.outerDiameter ** 2) / 4;
    primitives.push({ schema: 'fea-linear-load-primitive/v1', primitiveId: `${loadCaseId}-INSULATION-${entry.elementId}`, kind: 'DISTRIBUTED_WEIGHT', elementId: entry.elementId, weightComponent: 'INSULATION', massPerUnitLength: insulationMass, densityEvidence: sourceEvidence({ sourceId: `${SOURCE_ID}-INSUL_DENSITY`, sourceRevision: `${entry.segment.id}:${analysis.insulationDensity}` }), geometryEvidence: sourceEvidence({ sourceId: `${SOURCE_ID}-INSUL_THICK`, sourceRevision: `${entry.segment.id}:${analysis.insulationThickness}` }), sourceEvidence: sourceEvidence({ sourceId: `${SOURCE_ID}-INSULATION`, sourceRevision: entry.segment.id }) });
...
function displacementStressResults(authorities, sustained, operating, codeAuthorities) {
  ...
  return authorities.modelEntries.flatMap((entry) => ['I', 'J'].map((end) => {
    ...
    return compileCodeResult({ codeProfile: profile, editionDataset: dataset, stressFactorSet: unityStressFactors(entry.segment.id), category: 'DISPLACEMENT_STRESS_RANGE', codePointId: `${entry.segment.id}.${end}`, componentId: entry.segment.id, combinationId: 'BM1-OPERATING-MINUS-SUSTAINED', frameElementRecord: frame, sectionResolution: entry.section, materialResolution: authorities.material, localAction, pressureStressContribution: null, coldTemperature: { value: INSTALLATION_TEMPERATURE, source: 'M020 explicit installation-temperature authority' }, occasionalCategoryId: null });
  }));
}
...
function buildReport(authorities, sustained, operating, code) {
  return Object.freeze({ schema: 'm020-bm1-inputxml-analysis-report/v1', sourceSemanticHash: authorities.source.semanticHash, conditionedTopologyHash: authorities.conditioned.semanticHash,
    ...
    limitations: ['InputXML declares no installation temperature or thermal expansion coefficient; M020 explicitly uses 293.15 K and 1.17e-5 1/K.', 'BM1 bend spans declare internal CAESAR station nodes that are not inserted by the current adapter; they remain finite chord frame spans and are not reported as resolved arcs.', 'ALLOWABLESTRESS values in the live file are sentinel-only; B-4 results use a clearly labelled non-qualification screening allowable.', 'One-way +Y restraints are represented by their engaged linear fixed-UY state.', 'Insulation self-weight (weightComponent INSULATION) is now included in gravity for every element; IX-S1-IX-S3 insulation density was Owner-confirmed as 2100 kg/m3 (2026-08-04, matching every other insulated element in the model) and added directly to the live InputXML rather than left unset.', 'reaction.* values in this report are the reaction applied BY the restraint TO the structure (the standard structural-analysis convention used throughout this repository). CAESAR II RESTRAINT_REPORT XML exports the equal-and-opposite force applied BY the pipe TO the restraint hardware. Comparing the two directly requires negating one side; verified empirically against BM1_CIIOutput.xml CASE 4 (SUS) — displacement.* signs already agree directly with no negation needed.'],
