...
function buildReport(authorities, sustained, operating, code) {
  return Object.freeze({ schema: 'm020-bm1-inputxml-analysis-report/v1', sourceSemanticHash: authorities.source.semanticHash, conditionedTopologyHash: authorities.conditioned.semanticHash,
    ...
    limitations: ['InputXML declares no installation temperature or thermal expansion coefficient; M020 explicitly uses 293.15 K and 1.17e-5 1/K.', 'BM1 bend spans declare internal CAESAR station nodes that are not inserted by the current adapter; they remain finite chord frame spans and are not reported as resolved arcs.', 'ALLOWABLESTRESS values in the live file are sentinel-only; B-4 results use a clearly labelled non-qualification screening allowable.', 'One-way +Y restraints are represented by their engaged linear fixed-UY state.', 'Insulation density is not declared on every source element, so insulation self-weight is reported as unavailable rather than partially expanded by the all-element gravity consumer.', 'reaction.* values in this report are the reaction applied BY the restraint TO the structure (the standard structural-analysis convention used throughout this repository). CAESAR II RESTRAINT_REPORT XML exports the equal-and-opposite force applied BY the pipe TO the restraint hardware. Comparing the two directly requires negating one side; verified empirically against BM1_CIIOutput.xml CASE 4 (SUS) — displacement.* signs already agree directly with no negation needed.'],
