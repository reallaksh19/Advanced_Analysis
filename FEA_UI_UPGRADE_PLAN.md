...
> **Implementation status (2026-07-26): COMPLETE.** UI-0 through UI-8 are
> implemented in the repository. The executable acceptance gate is
> `npm run gate`; the completion matrix and measured evidence are in
> `reports/fea-ui-upgrade-completion.md`. The original review and wave
> specifications below are retained as the design basis.
