...
const REAL_MANIFEST_URL = new URL('./fixtures/topology-edit/1885s/fixture-manifest.json', import.meta.url);
const LARGE_MANIFEST_URL = new URL('./fixtures/topology-edit/large-model/fixture-manifest.json', import.meta.url);
const DATASET_URL = new URL('../benchmarks/Sjson.json', import.meta.url);
const PROJECT_DATA_URL = new URL('../project-data/1885s-project-data-profile.json', import.meta.url);
const REAL_PROJECT_RUNNER_URL = new URL('../scripts/run-advanced-tab-benchmarks.mjs', import.meta.url);
const REAL_PROJECT_REPORT_URL = new URL('../public/qualification/advanced-tab-benchmarks.md', import.meta.url);
...
test('M005 certifies repository-owned 1885S data through the real production adapters', async () => {
  ...
  assert.equal(supportSites.summary.sourceSupportRecordCount, manifest.expected.sourceSupportRecordCount);
  assert.equal(supportSites.summary.supportAssemblyCount, manifest.expected.supportAssemblyCount);
  assert.equal(supportSites.summary.physicalLocationCount, manifest.expected.physicalLocationCount);
  ...
  const kinds = new Map();
  for (const row of [...renderModel.physicalPrimitives, ...renderModel.supportOverlayPrimitives]) {
...
test('M005/M006 retain the portable 25,600-component fixture as supplemental stress evidence', async () => {
  ...
  const browserSpec = await readFile(new URL('./topology-edit-wave5-browser.spec.mjs', import.meta.url), 'utf8');
  const harness = await readFile(new URL('./topology-edit-wave5-browser-harness.js', import.meta.url), 'utf8');
  assert.match(browserSpec, /componentCount:\s*25_600/u);
  assert.match(harness, /TopologyEditWave5BrowserEvidence\.v3/u);
  ...
  assert.match(harness, /ORIENTATION_TOP_SYNC/u);
  assert.match(harness, /ORIENTATION_PROJECTION_SYNC/u);
