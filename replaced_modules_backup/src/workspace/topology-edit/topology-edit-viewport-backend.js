...
export class TopologyEditViewportBackend {
  ...
  buildMeshGroup(group, elements = [], colorHex = 0x0284c7, opacity = 1, markerSize = 10) {
    ...
    // Default geometry for plain node markers (tiny junction dots).
    const nodeGeometry = new THREE.SphereGeometry(markerSize, radialSegments, Math.max(6, Math.floor(radialSegments * 0.75)));
    if (valid.length >= 500 && valid.every((element) => !element.primitiveValue && positive(element.sizeMm) === null)) {
      this.buildInstancedMarkers(group, valid, nodeGeometry, material);
      return;
    }
    for (const element of valid) {
      // If the element carries a full governed primitive (TEE_JUNCTION, OLET_BRANCH,
      // JUNCTION_MARKER), dispatch through materializeTopologyEditPrimitive so the
      // correct Three.js geometry builder (addTee, addOlet, SphereGeometry) is used.
      if (element.primitiveValue) {
        try {
          const result = materializeTopologyEditPrimitive(element.primitiveValue, {
            material,
            radialSegments,
            markerSize,
            pickUserData: element.pickTarget ? pickUserData(element) : {},
          });
          result.object.position.set(0, 0, 0);
          group.add(result.object);
        } catch (e) {
          // Degraded fallback: render a small sphere so the pick target is preserved.
          const mesh = new THREE.Mesh(
            new THREE.SphereGeometry(markerSize * 0.5, radialSegments, Math.max(6, Math.floor(radialSegments * 0.75))),
            material,
          );
      ...
      // Plain node element: render as a tiny sphere with sizeMm if available.
      const size = positive(element.sizeMm) ?? markerSize;
      const elementGeometry = size === markerSize
        ? nodeGeometry
