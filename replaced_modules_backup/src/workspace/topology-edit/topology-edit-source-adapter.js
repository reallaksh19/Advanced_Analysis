...
function buildSupports(dataset, topologyGraph, portToNode, attachmentModel, restraintModel) {
  ...
  return dataset.entities
    ...
    .map((entity) => {
      ...
      const attr = entity.properties?.attributes || {};
      const rawOrigin = geom.center || geom.start || attr.POS || attr.APOS || null;
      const origin = rawOrigin && [rawOrigin.x, rawOrigin.y, rawOrigin.z].every(v => Number.isFinite(Number(v)))
        ? { x: Number(rawOrigin.x), y: Number(rawOrigin.y), z: Number(rawOrigin.z) }
        : null;
