...
export function createDimensionAuthority(options = {}) {
  ...
  function resolveOutsideDiameter(evidence = {}, context = {}) {
    ...
    const bore = finitePositive(evidence.boreMm ?? evidence.insideDiameterMm ?? evidence.dimensions?.boreMm ?? evidence.nominalDiameterMm ?? evidence.diameterMm);
    ...
    if (bore !== null && wall !== null) {
      ...
      rows.push(candidate(
        bore,
        'DERIVED_NOMINAL_BORE_AS_OD',
        evidence.sourceEvidenceId || 'nominal_bore',
        50,
        'NOMINAL_BORE_USED_AS_OD_FOR_VISUALIZATION',
      ));
    }
