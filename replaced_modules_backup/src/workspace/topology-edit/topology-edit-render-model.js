...
export function projectVisualGeometryToViewport(model, canonicalTopology) {
  // Raw topology nodes render as tiny junction-dot markers (sizeMm = 3).
  // They serve as pick targets and spatial anchors; pipe/fitting geometry
  // comes entirely from segments and full-primitive elements below.
  const nodes = (canonicalTopology?.nodes || []).map((node) => ({
    id: node.id, entityId: node.id, type: 'node',
  ...
  // Only point-type primitives (no start/end span) go to elements;
  // span primitives (PIPE_CYLINDER, ELBOW_ARC, FLANGE_DISC, etc.) are
  // already fully represented in segments via primitiveToSegments.
...
function deriveEdge(edge, nodes, input, policy) {
  ...
  if (['PIPE', 'STRAIGHT', 'STRAIGHT_ELEMENT', 'TEE', 'OLET'].includes(type)) {
    return derivePipe(edge, type, start, end, evidence, input.dimensionAuthority, policy);
  }
...
function deriveElbow(edge, start, end, evidence, authority, policy) {
  ...
  const validArc = center && radius && nearlyEqual(distance(center, end), radius, Math.max(policy.chordErrorMm, (radius || 100) * 0.1, 5));
  const diagnostics = diagnosticsFromDimension(edge.id, diameter);
...
// Kinds that are point-only (no start/end span): they need a 3-D mesh at
// their center/position.  All span primitives (PIPE_CYLINDER, ELBOW_ARC,
// FLANGE_DISC, GASKET_DISC, CONICAL_REDUCER, VALVE_BODY, DIAGNOSTIC_CENTERLINE)
// already appear in segments via primitiveToSegments and must NOT generate an
// additional element here (that was the source of the 236-sphere problem).
const POINT_ONLY_KINDS = new Set(['JUNCTION_MARKER', 'TEE_JUNCTION', 'OLET_BRANCH']);
function primitiveToElements(primitive) {
  if (!POINT_ONLY_KINDS.has(primitive.kind)) return [];
  ...
  return [{
    ...
    y: position.y,
    z: position.z,
    // Embed full primitive so buildMeshGroup can call materializeTopologyEditPrimitive.
...
function normalizedType(value) {
  ...
  return ({
    BEND: 'ELBOW', ELBO: 'ELBOW', ELCO: 'ELBOW',
    FLAN: 'FLANGE', FLANGE: 'FLANGE',
    VALV: 'VALVE', VALVE: 'VALVE',
    GASK: 'GASKET', GASKET: 'GASKET',
    REDU: 'REDUCER', REDUCER: 'REDUCER',
    REDUCING_TEE: 'TEE', TEE: 'TEE',
    WELDOLET: 'OLET', SOCKOLET: 'OLET', OLET: 'OLET',
    INST: 'INSTRUMENT', INSTRUMENT: 'INSTRUMENT',
    TUBI: 'PIPE', TUBE: 'PIPE', BRANCH: 'PIPE', STRAIGHT: 'PIPE', STRAIGHT_ELEMENT: 'PIPE',
  })[token] || token;
}
