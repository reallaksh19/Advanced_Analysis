...
function buildComponentEvidence(dataset) {
  ...
  for (const entity of dataset?.entities ?? []) {
    ...
    const nominalBore = firstFinite(
      entity.nominalDiameterMm,
      attributes.ABORE, attributes.LBORE, attributes.ABOR, attributes.HBOR, attributes.TBOR, attributes.BORE,
      attributes.outsideDiameterMm, attributes.OUTSIDE_DIAMETER, attributes.DIAMETER
    );
    let center = finitePoint(entity.properties?.geometry?.center);
    let centerlineRadiusMm = firstFinite(
      entity.properties?.geometry?.bendRadiusMm,
      attributes.centerlineRadiusMm, attributes.CENTERLINE_RADIUS, attributes.BEND_RADIUS, attributes.RADI
    );
    const type = String(entity.entityType || '').toUpperCase();
    if ((!centerlineRadiusMm || centerlineRadiusMm === 0) && ['ELBOW', 'ELBO', 'ELCO', 'BEND'].includes(type) && nominalBore) {
      centerlineRadiusMm = nominalBore * 1.5;
    }
    if (['ELBOW', 'ELBO', 'ELCO', 'BEND'].includes(type)) {
      const arcCenter = deriveEvidenceElbowCenter(entity, attributes, centerlineRadiusMm);
      if (arcCenter) center = arcCenter;
    }
    evidence[entity.entityId] = {
      workspaceEntityIds: [entity.entityId], sourcePath: entity.sourcePath,
      outsideDiameterMm: firstFinite(entity.outsideDiameterMm, attributes.outsideDiameterMm, attributes.OUTSIDE_DIAMETER, nominalBore),
      boreMm: firstFinite(entity.boreMm, attributes.boreMm, attributes.BORE, nominalBore),
      wallThicknessMm: firstFinite(entity.wallThicknessMm, attributes.wallThicknessMm, attributes.WALL_THICKNESS),
      centerlineRadiusMm,
      center, reducerType: attributes.reducerType ?? attributes.REDUCER_TYPE,
      startOutsideDiameterMm: firstFinite(attributes.startOutsideDiameterMm, attributes.START_OUTSIDE_DIAMETER, attributes.ABORE, attributes.ABOR, nominalBore),
      endOutsideDiameterMm: firstFinite(attributes.endOutsideDiameterMm, attributes.END_OUTSIDE_DIAMETER, attributes.LBORE, nominalBore),
      eccentricOffsetDirection: finitePoint(attributes.eccentricOffsetDirection),
      branchOutsideDiameterMm: firstFinite(attributes.branchOutsideDiameterMm, attributes.BRANCH_OUTSIDE_DIAMETER, attributes.HBOR, attributes.TBOR),
...
function deriveEvidenceElbowCenter(entity, attributes, centerlineRadiusMm) {
  let start = finitePoint(attributes.APOS || attributes.start || entity.properties?.geometry?.start);
  let end = finitePoint(attributes.LPOS || attributes.end || entity.properties?.geometry?.end);
  if (!start || !end) {
    const ports = entity.properties?.ports || entity.ports || [];
    const p1 = ports.find(p => p.role === 'start' || String(p.key).includes('arrive')) || ports[0];
    const p2 = ports.find(p => p.role === 'end' || String(p.key).includes('leave')) || ports[1];
    if (p1?.position) start = finitePoint(p1.position);
    if (p2?.position) end = finitePoint(p2.position);
  }
  if (!start || !end) return undefined;
  const oriStr = String(attributes.ORI || '');
  const CARDS = { E: [1, 0, 0], W: [-1, 0, 0], N: [0, 1, 0], S: [0, -1, 0], U: [0, 0, 1], D: [0, 0, -1] };
  const m = oriStr.match(/Y\s+is\s+(\w+)\s+and\s+Z\s+is\s+(\w+)/i);
  let inflowDir = null;
  if (m && CARDS[m[1].toUpperCase()] && CARDS[m[2].toUpperCase()]) {
    const ly = CARDS[m[1].toUpperCase()];
    const lz = CARDS[m[2].toUpperCase()];
    inflowDir = { x: ly[1] * lz[2] - ly[2] * lz[1], y: ly[2] * lz[0] - ly[0] * lz[2], z: ly[0] * lz[1] - ly[1] * lz[0] };
  }
  if (!inflowDir) {
    ...
    if (nonZeros.length === 2) {
      return { x: dx ? end.x : start.x, y: dy ? end.y : start.y, z: dz ? end.z : start.z };
    }
    return undefined;
  }
  ...
  const R = Math.abs(vec.x * inflowDir.x + vec.y * inflowDir.y + vec.z * inflowDir.z) || centerlineRadiusMm;
  if (!(R > 0)) return undefined;
  return { x: end.x - inflowDir.x * R, y: end.y - inflowDir.y * R, z: end.z - inflowDir.z * R };
}
...
function firstFinite(...values) {
  for (const value of values) {
    ...
    if (!Number.isFinite(number) && typeof value === 'string') {
      ...
      if (match) number = Number(match[0]);
    }
